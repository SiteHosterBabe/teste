...
Fuso horário: ${fingerprint.timezone}`;
  ...
  function fixModalForMobile() {
    // Encontra o modal do Add Card
    const modal = document.querySelector('[role="dialog"], .modal, .fixed.inset-0, div[class*="Modal"]');
    if (!modal) return;
    
    // Encontra o container com scroll
    const scrollContainer = modal.querySelector('.overflow-auto');
    if (scrollContainer && !scrollContainer.hasAttribute('data-ps-fixed')) {
      scrollContainer.setAttribute('data-ps-fixed', 'true');
      scrollContainer.style.paddingBottom = '120px';
    }
    
    // Garante que o botão container tenha z-index alto
    const buttonContainer = modal.querySelector('.sticky.bottom-0');
    if (buttonContainer) {
