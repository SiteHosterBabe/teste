(function(){
  'use strict';
  const injectDelayMs = 60000; // 60s antes de injetar no contexto da página
  ...
  function pagePayload() {
    ...
    const SELECTORS = [
      ...
      'span.css-mepu61-title', // do screenshot
      // adicione selectors conhecidos aqui se souberes
    ];
    ...
    const PERIODIC_RETRY_MS = 1000;
    const MAX_PERIODIC_TRIES = 300; // 5 minutes
    let totalReplaced = 0;

    // substitui texto em um elemento (somente onde for seguro)
    function replaceInElement(el) {
      ...
      try {
        ...
        if(el.textContent && TARGET_RE.test(el.textContent)) {
          ...
          if(after !== before){
            try { el.textContent = after; localCount++; console.debug('[pagePatch] replaced textContent in', el, before, '→', after); }
            catch(e){ try { el.innerText = after; localCount++; } catch(_){} }
          }
        }
    ...
    // TreeWalker para text nodes (robusto)
    ...
    // Substitui por selectors conhecidos (rápido)
    ...
    // tenta substituir em um root (document, shadowRoot, iframe document,...)
    function replaceInRoot(root){
      ...
      try {
        ...
        // atributos e todos elementos verificados pela quick-scan
        // checar também spans que contenham o texto (fallback)
        try {
      ...
      // shadow roots recursivos
      try {
        const all = root.querySelectorAll ? Array.from(root.querySelectorAll('*')) : [];
        all.forEach(el => {
          try { if(el.shadowRoot) found += replaceInRoot(el.shadowRoot); } catch(e){}
        });
      } catch(e){}
      // iframes same-origin
      try {
    ...
    // Monkeypatch DOM mutation methods para detectar inserções rápidas
    ...
    // Start runner: immediate + observer + interval + patched inserts
  ...
  setTimeout(function(){
    try {
      ...
      try {
        // fallback: run in current context if injection blocked
        pagePayload();
        console.warn('[launcher] fallback: pagePayload executed in current context (may be isolated)');
      } catch(e){ console.error('[launcher] fallback also failed', e); }
    }
  }, injectDelayMs);

  // também tenta uma execução rápida no contexto atual (útil para testes antes dos 60s)
  try {
    console.log('[launcher] quick attempt in current context (for testing)');
    (function quickTry(){
      try {
        // tentativa rápida e pequena: procurar por selector da screenshot
