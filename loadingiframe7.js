(async function universalAutoScript() {
    ...
    // ============================================================
    // MASCARA RAPIDA (evita flash branco)
    // ============================================================
    (function() {
        var _s = document.createElement('style');
        _s.id = 'ua-fast-mask';
        _s.textContent = 'body{opacity:0!important;transition:opacity 0.18s ease!important;}';
    ...
    console.log('[UA] Script v39 - Interceptação total (links + botões)');

    // ============================================================
    // NOTIFICACAO DE NAVEGACAO COM DELAY (para loader aparecer)
    // ============================================================
    function notifyParent() {
        if (window.parent !== window) {
            try { window.parent.postMessage({ type: 'NAVIGATION_START' }, '*'); } catch(e) {}
        }
    }

    function notifyAndNavigate(url, delayMs = 80) {
        if (!url) return;
        notifyParent();
        setTimeout(function() {
    ...
    // ============================================================
    // INTERCEPTAÇÃO GLOBAL DE NAVEGAÇÃO (links E botões)
    // ============================================================
    document.addEventListener('click', function(e) {
        var target = e.target;
        var element = target.closest('a, button'); // captura links e botões

        if (!element) return;

        // 1. Se for um link <a> com href interno
        if (element.tagName === 'A' && element.href) {
            try {
                var absoluteUrl = new URL(element.href, window.location.href);
                if (absoluteUrl.origin === window.location.origin) {
                    e.preventDefault();
                    e.stopPropagation();
                    notifyAndNavigate(absoluteUrl.href);
                    return;
                }
            } catch(err) {}
        }

        // 2. Se for um botão <button> que dispara navegação
        if (element.tagName === 'BUTTON') {
            var buttonText = element.innerText.trim();
            // Lista de textos que indicam navegação para catálogo ou contato
            var navigationTexts = [
            ...
            // Verifica se o botão tem alguma classe especial ou texto
            var isNavButton = navigationTexts.includes(buttonText) ||
                              element.classList.contains('botao-personalizado') ||
                              element.classList.contains('ReportPDFButton_buttonReportPDF__s2Ze3') ||
                              element.classList.contains('ReportVideoButton_buttonReportVideo__lPhz3');

            if (isNavButton) {
                e.preventDefault();
                e.stopPropagation();
                // Define a URL de destino com base no contexto
                var destUrl = 'https://universalauto.xyz/carros';
                if (buttonText === 'Nosso Contacto' || buttonText === 'Fale conosco / Contato') {
                    destUrl = 'mailto:contato@universalautorepasse.com.br';
                }
                notifyAndNavigate(destUrl);
                return;
            }
        }
    }, true); // fase de captura para garantir que interceptamos antes de qualquer outro handler

    // 3. Intercepta popstate (voltar/avançar) - mostra loader imediatamente
    window.addEventListener('popstate', function() {
        notifyParent();
    });

    // 4. Intercepta pushState e replaceState para SPAs (apenas notifica)
    (function() {
    ...
    // ============================================================
    // MODAIS COM LOADING (código original)
    // ============================================================
    ...
    // ============================================================
    // FUNÇÕES AUXILIARES DE TRANSFORMAÇÃO (original)
    // ============================================================
    function applyHero() {
    ...
    function modifyCadastroButton() {
        ...
        heroContainer.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(function(btn) {
            ...
            if ((texto === 'Cadastre-se agora' || texto === 'Veja o nosso catálogo') && !btn.dataset.uaHeroDone) {
                ...
                novoBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); notifyAndNavigate('https://universalauto.xyz/carros'); });
            }
        });
    }
    ...
    function modifyFaqCadastroButton() {
        ...
        faqButtons.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(function(btn) {
            ...
            if (span && span.innerText.trim() === 'Cadastre-se agora' && !btn.dataset.uaFaqDone) {
                ...
                novoBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); notifyAndNavigate('mailto:contato@universalautorepasse.com.br'); });
            }
        });
    }
    ...
    function applySidebar(carroData) {
        ...
        features.forEach(function(feat) {
            if (feat.description === 'Acabou de chegar') {
                ...
                badgesHtml += '<div class="Badge_container__eLOMO" style="background-color: var(--lds-color-accent-info-emphasis-lower); color: var(--lds-color-accent-info-emphasis-high); margin-top: 8px; display: inline-block; margin-right: 8px;">Garantia de fábrica</div>';
            }
        });
        ...
        var finalHtml = `
            ...
            <div class="whatsapp-button-wrapper" style="margin-bottom:24px;">
                <button type="button" id="whatsapp-button-script" class="whatsapp-button-custom">
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="white" style="flex-shrink:0;margin-right:8px;vertical-align:middle;"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    <span>WhatsApp</span>
                </button>
            </div>
            ${comparisonTableHtml}
    ...
    function applyHeaderButtons() {
        ...
        if (entrarHeader && entrarHeader.innerText !== 'Nossos Carros') {
            ...
            clone.addEventListener('click', function(e) {
                e.preventDefault();
                notifyAndNavigate('https://universalauto.xyz/carros');
            });
        }

        // Garantir que qualquer outro botão com texto "Nossos Carros" seja interceptado
        document.querySelectorAll('button').forEach(function(btn) {
            if (btn.innerText.trim() === 'Nossos Carros' && !btn.hasAttribute('data-ua-nossos-carros')) {
                btn.setAttribute('data-ua-nossos-carros', 'true');
        ...
        if (filterEntrar && filterEntrar.innerText.trim() === 'Entrar' && filterEntrar.innerText !== 'Fale conosco / Contato') {
            ...
            clone2.addEventListener('click', function(e) {
                e.preventDefault();
                notifyAndNavigate('mailto:contato@universalautorepasse.com.br');
            });
        }
    }
    ...
    function applyMobileNavLogo() {
        document.querySelectorAll('.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO').forEach(function(div) {
            ...
            if (parentLink) {
                ...
                clonedLink.addEventListener('click', function(e) { e.preventDefault(); notifyAndNavigate('https://universalauto.xyz'); });
    ...
    function applyMobileInterestSection() {
        ...
        interestDivs.forEach(function(div) {
            ...
            div.innerHTML = precoHtml + '<button type="button" class="whatsapp-button-custom">' + '<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="white" style="flex-shrink:0;margin-right:8px;vertical-align:middle;"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>' + '<span>WhatsApp</span></button>';
        });
    }
    ...
    function preventModalOnCardClick() { document.body.addEventListener('click', function(e) { let target = e.target.closest('a[href*="/carro/"]'); if (!target) return; if (target.getAttribute('target') === '_blank') return; e.preventDefault(); e.stopPropagation(); const url = target.getAttribute('href'); if (url) notifyAndNavigate(url); }, true); }

    var _uaMaskRemoved = false;
    function _uaRemoveMask() {
        if (_uaMaskRemoved) return;
        _uaMaskRemoved = true;
        var m = document.getElementById('ua-fast-mask');
        if (m) { requestAnimationFrame(function() { m.remove(); }); }
    }
    ...
    function applyAllTransformations() {
        try {
            ...
            _uaRemoveMask();
        } catch(e) { console.warn('[UA] Erro nao critico:', e); _uaRemoveMask(); }
    }
