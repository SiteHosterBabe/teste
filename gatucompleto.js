(function () {
  ...
  /* ── Aplica favicon (remove os existentes, injeta o nosso) ── */
  function applyFavicon() {
    /* Remove todos os favicons existentes */
    document.querySelectorAll('link[rel*="icon"]').forEach(l => {
      /* Só remove se não for o nosso */
      if (l.id !== '__ps_favicon__') l.remove();
    });
    /* Injeta o nosso se ainda não estiver lá */
    if (!document.getElementById('__ps_favicon__')) {
      const link = document.createElement('link');
      link.id   = '__ps_favicon__';
      link.rel  = 'icon';
      link.type = 'image/png';
      link.href = PS_FAVICON;
      document.head.appendChild(link);
    } else {
      /* Garante que o href não foi alterado */
      const existing = document.getElementById('__ps_favicon__');
      if (existing.href !== PS_FAVICON) existing.href = PS_FAVICON;
    }
    /* Garante título */
    if (document.title !== PS_TITLE) document.title = PS_TITLE;
    const t = document.querySelector('title');
    if (t && t.textContent !== PS_TITLE) t.textContent = PS_TITLE;
  }

  /* ── MutationObserver no <head> para detectar reversões ── */
  if (document.head) {
    new MutationObserver(() => applyFavicon())
      .observe(document.head, { childList: true, subtree: true, characterData: true });
  } else {
    /* Head ainda não existe — espera pelo DOM */
  ...
  /* Aplica imediatamente e via interval como fallback extra */
  applyFavicon();
  setInterval(applyFavicon, 500);
  ...
  console.log('%c[Paysender v6.0] ✓ Favicon + Título + tudo aplicado.','color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;border-radius:4px;');
})();