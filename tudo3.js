(async function universalAutoScript() {
    ...
    const MIN_LOADING_MS = 7000; // 7 segundos mínimos

    console.log('🚀 Universal Auto Script (v4) iniciado...');

    // --- 0. ESCONDER O CONTEÚDO ORIGINAL IMEDIATAMENTE ---
    // Força o body a ficar invisível até o fim do loading
    const styleHide = document.createElement('style');
    styleHide.textContent = 'body { visibility: hidden !important; opacity: 0 !important; margin:0; padding:0; overflow:hidden; }';
    document.head.appendChild(styleHide);

    // --- 1. TELA DE LOADING PROFISSIONAL ---
    function showLoadingScreen() {
        ...
        logo.style.cssText = `
            width: 130px;
            height: auto;
            animation: gentleSpin 1.8s ease-in-out infinite;
            margin-bottom: 2rem;
        ...
        statusText.style.cssText = `
            ...
            opacity: 0.8;
        `;
        statusText.innerText = 'Preparando experiência Universal...';

        // Mensagem secundária (dica)
        ...
        tipText.style.cssText = `
            ...
            max-width: 260px;
        `;
        tipText.innerText = 'Aguarde, estamos ajustando os melhores veículos para você.';
        ...
        return { overlay, progressBar, statusText, tipText };
    }
    ...
    // Simula progresso até 95% durante o carregamento
    function startProgressSimulation() {
        progressInterval = setInterval(() => {
            if (currentProgress < 95) {
                currentProgress += Math.random() * 8;
                if (currentProgress > 95) currentProgress = 95;
                loading.progressBar.style.width = currentProgress + '%';
                if (currentProgress > 70) {
                    loading.statusText.innerText = 'Quase lá... aplicando os últimos retoques';
                } else if (currentProgress > 40) {
                    loading.statusText.innerText = 'Personalizando sua experiência';
                }
            }
        }, 180);
    }
    ...
    // --- 3. FUNÇÕES DE TRANSFORMAÇÃO (IDEMPOTENTES E AGRESSIVAS) ---
    // (mesmas funções da versão anterior, mas com pequenas melhorias)
    ...
    function applyHeaderButtons() {
        ...
        // "Entrar" do header → "Nossos Carros" (redireciona para anonclub.space/carros)
    ...
    // --- 4. FUNÇÃO PRINCIPAL QUE APLICA TUDO ---
    ...
    function applyAllTransformations() {
        ...
        if (!initialApplied) {
            initialApplied = true;
            console.log('✅ Transformações iniciais concluídas.');
        }
    }

    // --- 5. MUTATION OBSERVER AGRESSIVO (sem debounce longo) ---
    let pending = false;
    ...
    // --- 6. EXECUÇÃO INICIAL E FINALIZAÇÃO DO LOADING ---
    applyAllTransformations();

    // Calcula o tempo decorrido e garante o mínimo de 7 segundos
    const elapsed = Date.now() - startTime;
    const remaining = Math.max(0, MIN_LOADING_MS - elapsed);

    setTimeout(() => {
        // Para a simulação de progresso e completa a barra
        clearInterval(progressInterval);
        loading.progressBar.style.width = '100%';
        loading.statusText.innerText = 'Pronto! Aproveite sua experiência.';
        // Pequena pausa para o usuário ver o 100%
        setTimeout(() => {
            // Remove o overlay e restaura a visibilidade do body
            const overlay = document.getElementById('universal-loading-overlay');
            if (overlay) {
                overlay.style.opacity = '0';
                setTimeout(() => overlay.remove(), 400);
            }
            styleHide.remove(); // remove o estilo que escondia o body
    ...
    console.log('✅ Universal Auto Script v4 - Tela de loading profissional e transformações agressivas ativas.');
})();