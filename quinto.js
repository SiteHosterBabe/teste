(async function universalAutoScript() {
    ...
    const LOADER_TIMEOUT_MS = 7000; // 7 segundos

    // Esconde o body imediatamente para evitar flash
    document.documentElement.style.visibility = 'hidden';

    console.log('🚀 Universal Auto Script (v6) iniciado...');

    // ============================================================
    // LOADER PREMIUM (substitui o antigo) - com lógica melhorada
    // ============================================================
    (function() {
        ...
        // === NOVA LÓGICA DE REMOÇÃO DO LOADER ===
        let transformationsCompleted = false;
        let startTime = Date.now();
        
        window.markTransformationsComplete = function() {
            transformationsCompleted = true;
        ...
        function removeLoader() {
            ...
            document.documentElement.style.visibility = ''; // revela o body
            if (loadingDiv && loadingDiv.parentNode) loadingDiv.parentNode.removeChild(loadingDiv);
            console.log("🏁 Loader premium removido.");
        }
        
        const checkInterval = setInterval(() => {
            if (transformationsCompleted && (Date.now() - startTime >= TIMEOUT_MS)) {
                clearInterval(checkInterval);
                removeLoader();
            }
        }, 100);
        
        // Fallback: remove após TIMEOUT_MS + 2 segundos no máximo
        setTimeout(() => {
            if (!loaderRemoved) {
                clearInterval(checkInterval);
                removeLoader();
        ...
        console.log("🚀 Loader premium ativo. Desaparecerá após transformações e 7 segundos.");
    })();
    ...
    // ============================================================
    // MUTATION OBSERVER + INTERVALO AGRESSIVO
    // ============================================================
    let debounceTimer;
    ...
    applyAllTransformations();
    // Notifica o loader que as transformações iniciais foram completadas
    ...
    console.log('✅ Universal Auto Script v6 - Loader premium integrado com sucesso.');
})();