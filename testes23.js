(async function universalAutoScript() {
    ...
    // ============================================================
    // MASCARA SINCRONA IMEDIATA - apenas visual, nao bloqueia DOM
    // ============================================================
    // REMOVIDA a mascara que ocultava o body, pois ela impedia o codigo
    // de ver e modificar o conteudo original. O loader agora e uma
    // sobreposicao pura via position:fixed, sem tocar na visibilidade
    // dos elementos originais do site.
    ...
    const LOADER_TIMEOUT_MS = 14000;   // ALTERADO: 14 segundos
    const WHATSAPP_BASE_MESSAGE = 'Olá! Tenho interesse em um veículo.';

    console.log('[UA] Script v36 - Loader como sobreposição, logo corrigido (14s loader)');

    // ============================================================
    // LOADER PREMIUM (14 segundos) - position:fixed puro, sem mascarar o DOM
    // ============================================================
    (function() {
        ...
        // position:fixed com z-index altissimo - sobrepoe o site mas nao esconde o DOM
    ...
    // ============================================================
    // SUBSTITUIÇÃO DO LOGO SEMINOVOS LOCALIZA (navbar link)
    // ============================================================
    // HTML fornecido: <a title="Seminovos Localiza" data-testid="home-page" class="Navbar_logoLink__6znXv" href="/">
    // Objetivo: trocar logo SVG + texto "Seminovos Localiza" por logo UA + "Universal Auto" em azul
    //           e redirecionar para https://universalauto.xyz/carros
    ...
    function replaceLocalizaNavbarLogo() {
        // Seleciona o link principal da navbar (desktop e mobile)
        var navLinks = document.querySelectorAll(
            'a[title="Seminovos Localiza"][data-testid="home-page"], a.Navbar_logoLink__6znXv'
        );
        navLinks.forEach(function(link) {
            if (link.hasAttribute('data-ua-logo-replaced')) return;
            link.setAttribute('data-ua-logo-replaced', 'true');

            // Atualiza titulo, href e remove data-testid para evitar reativacao
            link.setAttribute('title', 'Universal Auto');
            link.setAttribute('href', UA_REDIRECT);
            link.removeAttribute('data-testid');

            // Substitui todo o conteúdo interno (SVGs, DisplayContent wrappers, etc.)
            link.innerHTML = buildUALogoHTML();

            // Remove listeners antigos clonando o node
            var cloned = link.cloneNode(true);
            link.parentNode.replaceChild(cloned, link);

            // Adiciona redirect correto
    ...
    // ============================================================
    // SUBSTITUIÇÃO DO SVG LOCALIZA (viewBox 0 0 124 33) pelo logo UA
    // ============================================================
    // O SVG do documento 1 (viewBox="0 0 124 33") aparece dentro de
    // .DisplayContent_hideMobile divs e tambem no loader placeholder.
    // Substituimos qualquer SVG com esse viewBox por nossa imagem.

    function replaceLocalizaSVGInstances() {
        // Substitui SVGs com viewBox "0 0 124 33" (logo desktop Localiza)
        document.querySelectorAll('svg[viewBox="0 0 124 33"]').forEach(function(svg) {
            if (svg.hasAttribute('data-ua-svg-replaced')) return;
            // Nao substitui o loader SVG do carro
        ...
        // Substitui SVGs com viewBox "0 0 149 39" (variante do logo Localiza)
        document.querySelectorAll('svg[viewBox="0 0 149 39"]').forEach(function(svg) {
            if (svg.hasAttribute('data-ua-svg-replaced')) return;
            if (svg.closest('#premium-loader')) return;
            svg.setAttribute('data-ua-svg-replaced', 'true');
                ...
                if (hasPdf || hasVideo) {
                    ...
                    if (hasPdf) {
                        ...
                        pdfBtn.addEventListener('click', function(e) {
                            ...
                            showPdfModal(report.pdfUrl); return false;
                        });
                    ...
                    if (hasVideo) {
                        ...
                        videoBtn.addEventListener('click', function(e) {
                            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
                            showVideoModal(report.videoUrl); return false;
                        });
    ...
    function applySidebar(carroData) {
        ...
        if (hasPdf) {
            ...
            if (pdfBtn) { pdfBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); showPdfModal(report.pdfUrl); }); }
        }
        if (hasVideo) {
            var videoBtn = container.querySelector('.ReportVideoButton_buttonReportVideo__lPhz3');
            if (videoBtn) { videoBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); showVideoModal(report.videoUrl); }); }
        }
    }
        ...
        document.querySelectorAll('.VehicleTitle_container__mDy6V').forEach(function(tc) {
            ...
            if ((hasPdf || hasVideo) && !tc.nextElementSibling?.hasAttribute('data-ua-mobile-reports')) {
                ...
                if (hasPdf) {
                    ...
                    pdfBtn2.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); showPdfModal(report.pdfUrl); });
                    mobileReports.appendChild(pdfBtn2);
                }
                if (hasVideo) {
                    var videoBtn2 = document.createElement('button');
                    ...
                    videoBtn2.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); showVideoModal(report.videoUrl); });
    ...
    function applyLogo() {
        // Substitui o logo desktop antigo (viewBox 0 0 149 39 com #FF8026)
        ...
        // Chama as substituicoes genericas de SVG do Localiza
        replaceLocalizaSVGInstances();
        // Substitui o link da navbar Seminovos Localiza
    ...
    function applyMobileNavLogo() {
        document.querySelectorAll('.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO').forEach(function(div) {
            var svgEl = div.querySelector('svg[viewBox="0 0 39 39"], svg[viewBox="0 0 32 32"]');
            if (!svgEl) return;
            ...
            if (parentLink) {
                ...
                clonedLink.addEventListener('click', function(e) { e.preventDefault(); window.location.href = UA_REDIRECT; });
    ...
    function updateFavicon() { const newFaviconUrl = UA_LOGO_URL; let existingFavicon = document.querySelector('link[rel="shortcut icon"], link[rel="icon"]'); if (existingFavicon) { if (existingFavicon.getAttribute('href') !== newFaviconUrl) existingFavicon.setAttribute('href', newFaviconUrl); } else { const newLink = document.createElement('link'); newLink.rel = 'shortcut icon'; newLink.href = newFaviconUrl; newLink.type = 'image/png'; document.head.appendChild(newLink); } }
    ...
    function applyAllTransformations() {
        try {
            ...
            // applyLogo ja inclui replaceLocalizaSVGInstances e replaceLocalizaNavbarLogo
            applyLogo();
            applyCarouselDots();
            injectGlobalCSS();
            replaceHeroImage();
    ...
    // ============================================================
    // REMOCAO DO LOADER (14 segundos obrigatórios)
    // O site já está modificado por baixo - o loader apenas é removido
    // ============================================================
    let loaderRemovedGlobal = false;
    const LOADER_DURATION = 14000;   // ALTERADO: 14 segundos
    ...
    function removeLoader() {
        ...
        if (loadingDiv) {
            ...
            // Fallback para remover caso transitionend nao dispare
            setTimeout(() => { if (loadingDiv.parentNode) loadingDiv.parentNode.removeChild(loadingDiv); }, 700);
        }
    }

    // Loader dura exatamente 14 segundos, depois revela o site ja modificado
    setTimeout(removeLoader, LOADER_DURATION);
    ...
    // Aplica transformacoes IMEDIATAMENTE (enquanto loader ainda aparece)
