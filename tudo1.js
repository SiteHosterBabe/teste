(async function universalAutoScript() {
    ...
    console.log('🚀 Universal Auto Script (v2) iniciado...');

    // --- 0. Ocultar página até o fim da primeira execução (evita flash) ---
    const styleHide = document.createElement('style');
    styleHide.textContent = 'body { opacity: 0 !important; transition: opacity 0.2s; }';
    ...
    // --- Funções de modificação (idempotentes, podem ser chamadas várias vezes) ---
    ...
    // 2. Texto "Portal do Lojista" → "Universal Auto"
    ...
    // 3. Listagem: preços + botões "Tenho interesse"
    function applyListagem() {
        ...
        cards.forEach(card => {
            ...
            // Botão "Ver mais informações" → "Tenho interesse"
    ...
    // 4. Barra lateral (página de detalhes)
    function applySidebar(carroData) {
        ...
        if (!container) return;
        // Evita recriar se já está com o conteúdo correto (opcional)
    ...
    function applyHeaderButtons() {
        ...
        // "Entrar" do header → "Nossos Carros"
    ...
    function applyFooter() {
        // Remover blocos Localiza e Zarp
        ...
        // Substituir frotas → Universal site
    ...
    // 7. Textos genéricos (endereço, CNPJ, empresa, copyright)
    ...
    // 8. Logo principal (SVG → imagem + texto)
    function applyLogo() {
    ...
    function applyAllTransformations() {
        ...
        // Página de detalhes: pega o ID da URL e aplica a sidebar se houver dados
        ...
        // Revela o body na primeira execução
        if (primeiraExecucao) {
            primeiraExecucao = false;
            setTimeout(() => { styleHide.remove(); }, 100);
        }
    }

    // --- MutationObserver para reaplicar mudanças quando o DOM for alterado (SPA) ---
    let debounceTimer;
    ...
    applyAllTransformations();
    console.log('✅ Universal Auto Script v2 - Modificações aplicadas e observador ativo.');
})();