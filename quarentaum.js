(async function universalAutoScript() {
    ...
    // ============================================================
    // [FIX 1] MASCARA SINCRONA IMEDIATA (sempre injetada)
    // ============================================================
    ...
    const LOADER_TIMEOUT_MS = 3000; // Agora 3 segundos

    const WHATSAPP_BASE_MESSAGE = 'Olá! Tenho interesse em um veículo.';

    console.log('[UA] Universal Auto Script (v26 - Loader 3s + Mask 2s) iniciado...');

    // ============================================================
    // LOADER PREMIUM (duração exata de 3 segundos)
    // ============================================================
    (function() {
        const TIMEOUT_MS = LOADER_TIMEOUT_MS; // 3000
        ...
        console.log('[UA] Loader premium injetado no DOM (duração: 3s)');
        ...
        const progressInterval = setInterval(function() {
            if (progress < 100) {
                progress += 100 / (TIMEOUT_MS / 50); // 3s = 60 steps de 50ms -> 1.666% por step
        ...
        function startDashDrawingSequence(staggerDelayMs, drawDurationMs) {
            staggerDelayMs = staggerDelayMs || 80;
            drawDurationMs = drawDurationMs || 500;
        ...
        function initWheelStripeRotation() {
            ...
            function rotateStripes() {
                ...
                for (var idx = 0; idx < allStripes.length; idx++) {
                    var newOffset = stripeOffsets[idx] - 0.38;
    ...
    function applyMobileNavLogo() {
        ...
        mobileLogoDivs.forEach(function(div) {
            ...
            if (parentLink) {
                ...
                if (newDiv) {
                    newDiv.innerHTML = '<img data-ua-mobile-logo="1" src="https://universalautorepasse.com.br/wp-content/uploads/2024/10/Universal-Veiculos-e-Repasses-300x275.png" alt="Universal Auto" style="height:39px;width:auto;object-fit:contain;display:block;">';
                }
            } else {
                div.innerHTML = '<img data-ua-mobile-logo="1" src="https://universalautorepasse.com.br/wp-content/uploads/2024/10/Universal-Veiculos-e-Repasses-300x275.png" alt="Universal Auto" style="height:39px;width:auto;object-fit:contain;display:block;">';
            }
    ...
    function updateFavicon() {
        const newFaviconUrl = 'https://universalautorepasse.com.br/wp-content/uploads/2024/10/Universal-Veiculos-e-Repasses-300x275.png';
    ...
    // ============================================================
    // REMOCAO CONTROLADA: LOADER SAI APÓS 3s, MÁSCARA PERMANECE +2s
    // ============================================================
    let loaderRemovedGlobal = false;
    let maskRemoved = false;
    const loaderStartTime = Date.now();
    const LOADER_DURATION = 3000;  // 3 segundos
    const MASK_EXTRA_DURATION = 1000; // 2 segundos extras
    ...
    function removeLoaderAndScheduleMaskRemoval() {
        ...
        if (loadingDiv) {
            ...
            console.log('[UA] Loader removido após 3 segundos.');
        }
        // Agenda remoção da máscara após 2 segundos
        setTimeout(() => {
            if (!maskRemoved) {
                ...
                if (mask) {
                    mask.remove();
                    maskRemoved = true;
                    console.log('[UA] Máscara removida após +2 segundos (total 5s).');
                }
            }
        }, MASK_EXTRA_DURATION);
    }

    // Aguarda 3 segundos exatos e depois remove loader + agenda máscara
    setTimeout(() => {
        removeLoaderAndScheduleMaskRemoval();
        if (typeof window.markTransformationsComplete === 'function') window.markTransformationsComplete();
    }, LOADER_DURATION);

    // ============================================================
    // INICIALIZACAO DOS OBSERVADORES E TRANSFORMACOES
    // ============================================================
    observeModalInstant();
    ...
    console.log('[UA] Script v26 ativo. Loader: 3s, Máscara extra: 2s.');
})();