(function universalAutoScript() {
    ...
    const URL_JSON = 'https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/cristianoronaldo.json';
    ...
    console.log('[UA] Script v43 - Remove botão Colaborador + substituição de imagens de catálogo');
    ...
    (function blockAppPrompts() {
        // Remove meta tags que fazem o iOS sugerir abrir a app
        ['apple-itunes-app','google-play-app','application-name','al:ios:app_store_id',
         'al:ios:url','al:android:package','al:android:url'].forEach(name => {
            document.querySelectorAll('meta[name="'+name+'"],meta[property="'+name+'"]')
                .forEach(m => m.remove());
        });
        // Remove link rel="manifest" que pode iniciar um install prompt PWA
        document.querySelectorAll('link[rel="manifest"]').forEach(l => l.remove());
        // Bloqueia o evento beforeinstallprompt (banner de instalar PWA)
        window.addEventListener('beforeinstallprompt', e => { e.preventDefault(); e.stopImmediatePropagation(); }, true);
        // Bloqueia applinks / deep-links ao clicar em elementos com schema externo
        document.addEventListener('click', e => {
            const a = e.target.closest('a[href]');
            if (!a) return;
            const href = a.getAttribute('href') || '';
            if (/^(portaldolojista|localiza|intent|market):/.test(href)) {
    ...
    (function setupContactModal() {
        ...
        function buildModal() {
            ...
            document.getElementById('ua-modal-submit').addEventListener('click', () => {
                ...
                waMsg += `*Mensagem:* ${msg}\n\n`;
                waMsg += `_Mensagem enviada pelo site universalauto.xyz_`;
        ...
        // --- Intercepta o clique no botão original (ContactUs) ---
        document.addEventListener('click', e => {
            const btn = e.target.closest('[class*="ContactUs_buttonFixed"] button, [aria-label="Fale conosco"]');
            if (!btn) return;
            e.preventDefault();
