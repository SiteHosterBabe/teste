(async function universalAutoScript() {
    ...
    console.log('[UA] Universal Auto Script (v10 - modais e seção equipe) iniciado...');

    // ============================================================
    // LOADER PREMIUM (mesmo código, omitido por brevidade, mas mantido integralmente)
    // ============================================================
    (function() {
    ...
    function modifyLoginDialog() {
        // Detecta o modal/diálogo que aparece ao clicar em "Ver mais informações" em um card
        var dialog = document.querySelector('.LdsDialog-module_lds-dialog-container__Br8cE');
        if (!dialog) return;
        // Verifica se o conteúdo específico existe (para evitar modificar outros diálogos)
        var content = dialog.querySelector('.Catalog_dialogContent__WQLIT');
        if (!content) return;
        
        // Substituir o texto do parágrafo principal
        ...
        // Remover o botão "Já possui conta? Faça o login"
        ...
        // Modificar o botão "Cadastre-se" para "Preço Único" e redirecionar
    ...
    function isPageFullyTransformed() {
        ...
        // Verificar seção Equipe Especializada
        ...
        // Se existir algum diálogo aberto, verificar se foi modificado (opcional, pois pode não estar visível)
        var dialog = document.querySelector('.LdsDialog-module_lds-dialog-container__Br8cE');
        if (dialog && dialog.style.display !== 'none') {
            var dialogP = dialog.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
            if (dialogP && dialogP.innerText.trim() === 'Faça o Login ou cadastre-se para ter acesso a todas nossas ofertas') {
                console.log('[UA] Aguardando modificação do diálogo...');
                return false;
            }
        }
    ...
    console.log('[UA] Script v10 ativo. Mascara de pagina ativa. Loader aguarda transformacoes.');
})();