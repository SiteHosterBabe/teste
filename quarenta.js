(async function universalAutoScript() {
    ...
    // ============================================================
    // [FIX 1] MASCARA SINCRONA IMEDIATA
    // ============================================================
    ...
    console.log('[UA] Universal Auto Script (v25 - Final) iniciado...');

    // ============================================================
    // LOADER PREMIUM (com tempo mínimo de exibição)
    // ============================================================
    (function() {
        ...
        function runFullLoadingCycle() {
            ...
            restartTimeout = setTimeout(function() {
                if (!loaderInternalRemoved) { isCycling = false; runFullLoadingCycle(); }
            }, 4000);
        }
    ...
    // ============================================================
    // FUNCOES DE TRANSFORMACAO (COMPLETAS)
    // ============================================================
    function applyHero() {
    ...
    function applyMobileNavLogo() {
        var mobileLogoDivs = document.querySelectorAll('.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO');
    ...
    function applyMobileInterestSection() {
        ...
        interestDivs.forEach(function(div) {
            ...
            console.log('[UA] Secção de interesse mobile substituída.');
        });
    }
    ...
    // ============================================================
    // NOVAS FUNCOES: TITLE, META DESCRIPTION, FAVICON
    // ============================================================
    function updatePageTitle() {
        const originalTitle = document.title;
        if (originalTitle !== 'Universal Auto') {
            document.title = 'Universal Auto';
            console.log('[UA] Título da página alterado para "Universal Auto".');
        }
    }
    ...
    function updateMetaDescription() {
        ...
        if (metaDesc.getAttribute('content') !== newDescription) {
            metaDesc.setAttribute('content', newDescription);
            console.log('[UA] Meta description atualizada.');
        }
    }

    function updateFavicon() {
        const newFaviconUrl = 'https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/logoalternativo.png';
        let existingFavicon = document.querySelector('link[rel="shortcut icon"], link[rel="icon"]');
        if (existingFavicon) {
            if (existingFavicon.getAttribute('href') !== newFaviconUrl) {
                existingFavicon.setAttribute('href', newFaviconUrl);
                console.log('[UA] Favicon substituído.');
            }
        } else {
            ...
            console.log('[UA] Favicon adicionado.');
        }
    }

    // ============================================================
    // BLOQUEIO DO MODAL DE LOGIN NOS CARDS (REDIRECIONAMENTO DIRETO)
    // ============================================================
    function preventModalOnCardClick() {
        document.body.addEventListener('click', function(e) {
            ...
            if (url) {
                console.log('[UA] Redirecionando diretamente para:', url);
    ...
    // ============================================================
    // VERIFICACAO MINIMA (RAPIDA PARA REMOVER MASCARA)
    // ============================================================
    function isPageMinimallyTransformed() {
        const mask = document.getElementById('ua-page-mask');
        if (mask) return false;
        const loader = document.getElementById('premium-loader');
        if (loader && loader.style.opacity !== '0') return false;
        const desktopLogoOk = !document.querySelector('.DisplayContent_displayContent__I14hU.DisplayContent_hideMobile__K13cl svg[viewBox="0 0 149 39"]');
        const mobileLogoOk = !document.querySelector('.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO svg[viewBox="0 0 39 39"]');
        if (!desktopLogoOk || !mobileLogoOk) return false;
        if (document.querySelector('button[aria-label="abrir menu"]')) return false;
        return true;
    }

    // ============================================================
    // REMOCAO SEGURA COM TEMPO MINIMO DE LOADER
    // ============================================================
    ...
    function safeRemoveLoader() {
        ...
        if (elapsed < MIN_LOADER_DISPLAY_MS) {
            setTimeout(safeRemoveLoader, MIN_LOADER_DISPLAY_MS - elapsed);
            return;
        }
        ...
        console.log('[UA] Máscara e loader removidos após tempo mínimo.');
    }
    ...
    // Bloqueio do modal de login nos cards
    ...
    // Verificação periódica rápida
    let verificationCount = 0;
    const verifyInterval = setInterval(() => {
        applyAllTransformations();
        if (isPageMinimallyTransformed()) {
            clearInterval(verifyInterval);
            safeRemoveLoader();
            return;
        }
        verificationCount++;
        if (verificationCount > 15) { // 3 segundos (15 * 200ms)
            clearInterval(verifyInterval);
            console.warn('[UA] Timeout rápido: removendo máscara forçadamente.');
            safeRemoveLoader();
        }
    }, 200);
    ...
    console.log('[UA] Script v25 completo. Aguardando remoção do loader...');
})();