(async function universalAutoScript() {
    ...
    console.log('[UA] Script v37 - Interceptação total de navegação');
    ...
    function notifyAndNavigate(url, delayMs = 80) {
        if (pendingNavigation) return; // evita múltiplas navegações simultâneas
        pendingNavigation = url;
        notifyParent();
        setTimeout(function() {
            window.location.href = url;
            setTimeout(() => { pendingNavigation = null; }, 200);
        }, delayMs);
    }

    // ============================================================
    // INTERCEPTAÇÃO GLOBAL DE NAVEGAÇÃO (links, botões JS, popstate)
    // ============================================================

    // 1. Intercepta cliques em links (inclusive relativos)
    ...
    // 2. Intercepta navegação via JavaScript (window.location.href / assign)
    (function() {
        ...
        function intercept(url) {
            ...
            isIntercepting = true;
            notifyAndNavigate(url);
            setTimeout(() => { isIntercepting = false; }, 200);
        }

        // Redefine window.location.href
        if (originalHrefSetter) {
            Object.defineProperty(window.location, 'href', {
                set: function(value) { intercept(value); },
                get: function() { return originalHrefSetter ? window.location.href : ''; },
                configurable: true
            });
        }

        // Redefine window.location.assign
        window.location.assign = function(url) { intercept(url); };
    })();

    // 3. Intercepta popstate (voltar/avançar) - mostra loader imediatamente
    window.addEventListener('popstate', function() {
        notifyParent(); // o iframe pai já exibe o loader instantaneamente
        // Pequeno delay extra para garantir que o loader apareça antes do conteúdo
        setTimeout(() => {}, 10);
    });
