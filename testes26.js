(async function universalAutoScript() {
    ...
    console.log('[UA] Script v39 - Loader com proteção contra remoção prematura, 14s');

    // ============================================================
    // FUNÇÃO PARA CRIAR O LOADER (será chamada se ele sumir antes do tempo)
    // ============================================================
    let loaderCreatedAt = null;
    let loaderRemovalScheduled = false;
    let loaderProtectionInterval = null;
    ...
    function createLoaderElement() {
        ...
        // Animações do carro (simplificadas para evitar repetição de código)
    ...
    // ============================================================
    // INICIALIZAÇÃO DO LOADER COM PROTEÇÃO
    // ============================================================
    function initLoader() {
        ...
        // Monitorar se o loader foi removido prematuramente
        if (loaderProtectionInterval) clearInterval(loaderProtectionInterval);
        loaderProtectionInterval = setInterval(() => {
            const currentLoader = document.getElementById('premium-loader');
            if (!currentLoader && !loaderRemovalScheduled) {
                console.warn('[UA] Loader foi removido prematuramente! Recriando...');
                // Recriar loader
                if (loaderElement && loaderElement.parentNode) loaderElement.parentNode.removeChild(loaderElement);
                loaderElement = createLoaderElement();
                loaderCreatedAt = Date.now();
                // Reagendar remoção
                if (window._removeLoaderTimeout) clearTimeout(window._removeLoaderTimeout);
                window._removeLoaderTimeout = setTimeout(() => {
    ...
    function removeLoader() {
        ...
        console.log('[UA] Tentando remover loader após', elapsed, 'ms (esperado:', LOADER_TIMEOUT_MS, 'ms)');
        const loadingDiv = document.getElementById('premium-loader');
        if (loadingDiv) {
            // Limpar intervals e timeouts para evitar memory leak
            if (loadingDiv._intervals) {
                clearInterval(loadingDiv._intervals.intervalFrases);
                clearInterval(loadingDiv._intervals.progressInterval);
            }
            if (loadingDiv._timeouts && loadingDiv._timeouts.restartTimeout) clearTimeout(loadingDiv._timeouts.restartTimeout);
    ...
    // Iniciar loader
    initLoader();
    // Agendar remoção após 14 segundos
    window._removeLoaderTimeout = setTimeout(removeLoader, LOADER_TIMEOUT_MS);
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.warn('[UA] JSON não carregado.');
        }
    } catch (err) {
