(async function universalAutoScript() {
    ...
    console.log('[UA] Script v43 - Loader sem bloqueio, transformações imediatas, 14s');
    ...
    // ============================================================
    // FUNÇÃO PARA CRIAR O LOADER (com pointer-events: none)
    // ============================================================
    function createLoaderElement() {
        ...
        // IMPORTANTE: pointer-events: none permite cliques e interações passarem
    ...
    // ============================================================
    // INICIALIZAÇÃO DO LOADER (inserção imediata, sem bloqueio)
    // ============================================================
    function initLoader() {
        ...
        console.log('[UA] Loader inserido - sobreposição visual apenas');
    }

    // ============================================================
    // REMOÇÃO DO LOADER APÓS 14 SEGUNDOS
    // ============================================================
    function removeLoader() {
        if (loaderRemoved) return;
        loaderRemoved = true;
        console.log('[UA] Removendo loader após', LOADER_TIMEOUT_MS / 1000, 'segundos');
    ...
    // ============================================================
    // CARREGAMENTO DOS DADOS (JSON) em paralelo
    // ============================================================
    fetch(URL_JSON)
        ...
        .then(data => {
            ...
            console.log('[UA] JSON carregado com', todosVeiculos.length, 'veículos');
            // Aplicar listagem imediatamente após carregar os dados
            if (!isApplying) applyAllTransformations();
        })
        .catch(err => console.warn('[UA] JSON não carregado:', err));

    // ============================================================
    // FUNÇÕES DE TRANSFORMAÇÃO (TODAS AS MODIFICAÇÕES)
    // ============================================================
    ...
    // ============================================================
    // FUNÇÕES DE LISTAGEM E SIDEBAR (modais, PDF, vídeo)
    // ============================================================
    ...
    function applyAllTransformations() {
        ...
        } catch(e) { console.warn('[UA] Erro na transformação:', e); }
    ...
    // Aplica transformações imediatamente e repete a cada 300ms durante o loader
    applyAllTransformations();
    const transformInterval = setInterval(() => {
        if (!loaderRemoved) applyAllTransformations();
        else clearInterval(transformInterval);
    }, 300);
    
    // Agendar remoção do loader após 14 segundos
    removalTimeout = setTimeout(() => {
        clearInterval(transformInterval);
        removeLoader();
    }, LOADER_TIMEOUT_MS);
    
    console.log('[UA] Loader visível por', LOADER_TIMEOUT_MS/1000, 'segundos. Transformações ativas em segundo plano.');

    // Configurar observadores para reaplicar transformações se o DOM mudar
    observeModalInstant();
    ...
    const domObserver = new MutationObserver(() => {
        if (!loaderRemoved && !isApplying) applyAllTransformations();
    });
