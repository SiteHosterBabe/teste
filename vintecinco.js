(async function universalAutoScript() {
    ...
    // CORREÇÃO: Substituir título principal (h1) com seletor mais abrangente
    function replaceMainTitle() {
        // Procura qualquer h1 com a classe heading-xl que contenha o texto original
        var titulos = document.querySelectorAll('h1.LdsTypography-module_lds-typography--heading-xl__xfa9y');
    ...
    // ========== NOVAS FUNÇÕES PARA A SEÇÃO "COMO FUNCIONA" ==========
    function replaceHowItWorksTexts() {
        // 1. Substituir h2 "Faça seu cadastro"
        var h2Cadastro = Array.from(document.querySelectorAll('h2.LdsTypography-module_lds-typography--heading-md__QVgP4')).find(function(el) {
            return el.innerText.trim() === 'Faça seu cadastro';
        });
        if (h2Cadastro && h2Cadastro.innerText !== 'Acesse o nosso catálogo') {
            h2Cadastro.innerText = 'Acesse o nosso catálogo';
            console.log('[UA] h2 "Faça seu cadastro" substituído.');
        }

        // 2. Substituir p "Cadastre seu negócio e representantes gratuitamente!"
        var pCadastro = Array.from(document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK')).find(function(el) {
            return el.innerText.trim() === 'Cadastre seu negócio e representantes gratuitamente!';
        });
        if (pCadastro && pCadastro.innerText !== 'Faça a sua pesquisa e veja por que somos a escolha inteligente. Compare e poupe.') {
            pCadastro.innerText = 'Faça a sua pesquisa e veja por que somos a escolha inteligente. Compare e poupe.';
            console.log('[UA] Parágrafo "Cadastre seu negócio..." substituído.');
        }

        // 3. Substituir h2 "Acesse o portal"
        var h2Portal = Array.from(document.querySelectorAll('h2.LdsTypography-module_lds-typography--heading-md__QVgP4')).find(function(el) {
            return el.innerText.trim() === 'Acesse o portal';
        });
        if (h2Portal && h2Portal.innerText !== 'Conheça os detalhes') {
            h2Portal.innerText = 'Conheça os detalhes';
            console.log('[UA] h2 "Acesse o portal" substituído.');
        }

        // 4. Substituir p "Faça o login e descubra todas as funcionalidades da plataforma."
        var pPortal = Array.from(document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK')).find(function(el) {
            return el.innerText.trim() === 'Faça o login e descubra todas as funcionalidades da plataforma.';
        });
        if (pPortal && pPortal.innerText !== 'Clique no modelo e acesse os dados vitais do veículo: quilometragem, laudo cautelar, estado de conservação e todos os acessórios.') {
            pPortal.innerText = 'Clique no modelo e acesse os dados vitais do veículo: quilometragem, laudo cautelar, estado de conservação e todos os acessórios.';
            console.log('[UA] Parágrafo "Faça o login..." substituído.');
        }

        // 5. Substituir h2 "Escolha seus carros"
        var h2Escolha = Array.from(document.querySelectorAll('h2.LdsTypography-module_lds-typography--heading-md__QVgP4')).find(function(el) {
            return el.innerText.trim() === 'Escolha seus carros';
        });
        if (h2Escolha && h2Escolha.innerText !== 'Entre em contato') {
            h2Escolha.innerText = 'Entre em contato';
            console.log('[UA] h2 "Escolha seus carros" substituído.');
        }

        // 6. Substituir p "Ampla variedade de modelos e marcas para o seu negócio."
        var pEscolha = Array.from(document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK')).find(function(el) {
            return el.innerText.trim() === 'Ampla variedade de modelos e marcas para o seu negócio.';
        });
        if (pEscolha && pEscolha.innerText !== 'Esclareça as suas dúvidas sobre crédito, especificações técnicas ou agende uma visita. A nossa equipa está pronta para o ajudar em todas as etapas da sua compra.') {
            pEscolha.innerText = 'Esclareça as suas dúvidas sobre crédito, especificações técnicas ou agende uma visita. A nossa equipa está pronta para o ajudar em todas as etapas da sua compra.';
            console.log('[UA] Parágrafo "Ampla variedade..." substituído.');
        }

        // 7. Substituir h2 "Compre online"
        var h2Compre = Array.from(document.querySelectorAll('h2.LdsTypography-module_lds-typography--heading-md__QVgP4')).find(function(el) {
            return el.innerText.trim() === 'Compre online';
        });
        if (h2Compre && h2Compre.innerText !== 'Agendar visita') {
            h2Compre.innerText = 'Agendar visita';
            console.log('[UA] h2 "Compre online" substituído.');
        }

        // 8. Substituir p "Ampla variedade de modelos e marcas para o seu negócio.Todo o processo de compra 100% online e seguro!"
        var pCompre = Array.from(document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK')).find(function(el) {
            return el.innerText.trim() === 'Ampla variedade de modelos e marcas para o seu negócio.Todo o processo de compra 100% online e seguro!';
        });
        if (pCompre && pCompre.innerText !== 'Solicite um agendamento personalizado com os nossos especialistas e venha avaliar, em detalhe e presencialmente, toda a qualidade dos nossos veículos.') {
            pCompre.innerText = 'Solicite um agendamento personalizado com os nossos especialistas e venha avaliar, em detalhe e presencialmente, toda a qualidade dos nossos veículos.';
            console.log('[UA] Parágrafo "Ampla variedade... online e seguro!" substituído.');
        }
    }
    ...
    function applyAllTransformations() {
        try {
            ...
            replaceMainTitle();      // corrigido
            modifyVerMaisCarrosButton();
            modifyHowItWorksBackground();
            modifySeparatorsBackground();
            modifyHowItWorksCadastroButton();
            replaceHowItWorksTexts(); // nova função com todas as substituições da seção "Como funciona"
