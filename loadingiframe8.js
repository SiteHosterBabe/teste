(async function universalAutoScript() {
    ...
    // ============================================================
    // NOTIFICACAO DE NAVEGACAO (postMessage para iframe pai)
    // ============================================================
    var UA_NAV_DELAY = 80; // ms de espera para o loader aparecer antes de navegar
    ...
    (function() {
        ...
        // Intercepta cliques em links internos com delay na navegacao
        // para garantir que o loader aparece ANTES da pagina mudar
        document.addEventListener('click', function(e) {
            var link = e.target.closest('a');
            if (!link) return;
            var href = link.getAttribute('href') || '';
            var fullHref = link.href || '';
            // Ignora: externos, mailto, ancora, javascript
            if (!href && !fullHref) return;
            if (href.startsWith('mailto:') || href.startsWith('#') || href.startsWith('javascript:')) return;
            if (fullHref.startsWith('mailto:')) return;
            var isInternal = href.startsWith('/') || fullHref.startsWith(location.origin);
            if (!isInternal) return;
            var target = link.getAttribute('target');
            if (target === '_blank') return;
            // Notifica o loader instantaneamente
            notifyParent();
            // Previne navegacao imediata e aguarda o loader aparecer no iframe
            e.preventDefault();
            e.stopPropagation();
            var dest = fullHref || href;
            setTimeout(function() { window.location.href = dest; }, UA_NAV_DELAY);
        }, true);

        // Intercepta pushState / replaceState (SPA navigation - nao precisa delay)
        var _push = history.pushState;
        var _replace = history.replaceState;
        history.pushState = function() { notifyParent(); return _push.apply(this, arguments); };
        history.replaceState = function() { notifyParent(); return _replace.apply(this, arguments); };
        window.addEventListener('popstate', function() { notifyParent(); });
        window.__uaNotifyNavigation = notifyParent;
    })();

    // Navega com delay para garantir que o loader do iframe aparece primeiro
    function notifyAndNavigate(url) {
        if (window.__uaNotifyNavigation) window.__uaNotifyNavigation();
        if (!url || url.startsWith('mailto:')) {
            window.location.href = url || '';
            return;
        }
        setTimeout(function() { window.location.href = url; }, UA_NAV_DELAY);
    }
    ...
    function applyHeaderButtons() {
        ...
        if (entrarHeader && entrarHeader.innerText !== 'Nossos Carros') {
            ...
            clone.addEventListener('click', function(e) { e.preventDefault(); notifyAndNavigate('https://universalauto.xyz/carros'); });
        }
        ...
        if (filterEntrar && filterEntrar.innerText.trim() === 'Entrar' && filterEntrar.innerText !== 'Fale conosco / Contato') {
            ...
            clone2.addEventListener('click', function(e) { e.preventDefault(); notifyAndNavigate('mailto:contato@universalautorepasse.com.br'); });
        }
    }
