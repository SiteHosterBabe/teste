(function universalAutoScript() {
    ...
    // CONFIGURACOES
    // ============================================================
    const WHATSAPP_NUMBER  = '5511992904812';
    const URL_JSON = 'https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/ministro26.json';
    const ACRESCIMO = 3000;
    const LOADER_TIMEOUT_MS = 8000;   // 8 segundos
    const WHATSAPP_BASE_MESSAGE = 'Olá! Vi este carro no site da loja e tenho interesse. Ainda está disponível? Poderia me passar mais detalhes e condições?';

    console.log('[UA] Script v45 - Substituição de número de telefone da loja');
    ...
    const UA_REDIRECT  = 'https://universalautorepasse.com';
    ...
    function applyListagem() {
        ...
        try {
            ...
            cards.forEach(card => {
                ...
                if (!veiculo) {
                    // Carro não encontrado no JSON — esconde o card sem remover do DOM
                    // (remover do DOM aciona o MutationObserver em loop infinito e trava o browser)
                    const rootCard = link.closest('[class*="VehicleCard_paper"]') || card;
                    if (!rootCard.hasAttribute('data-ua-hidden')) {
                        rootCard.setAttribute('data-ua-hidden', 'true');
                        rootCard.style.setProperty('display', 'none', 'important');
                    }
                    return;
                }

                // Marca o card como processado para o CSS o revelar (anti-flash)
                const rootCard = link.closest('[class*="VehicleCard_paper"]') || card;
                rootCard.setAttribute('data-ua-done', 'true');
    ...
    function injectGlobalCSS() {
        ...
        style.textContent = `
            /* ===== ANTI-FLASH: esconde cards até o script os processar ===== */
            /* Qualquer card que ainda não tenha o atributo data-ua-done fica invisível */
            [class*="VehicleCard_paper"]:not([data-ua-done]):not([data-ua-hidden]) {
                opacity: 0 !important;
                pointer-events: none !important;
            }
            /* Quando o script marca o card como processado, aparece com transição suave */
            [class*="VehicleCard_paper"][data-ua-done] {
                opacity: 1 !important;
                pointer-events: auto !important;
                transition: opacity 0.18s ease !important;
            }
            /* Cards a esconder ficam completamente ocultos */
            [data-ua-hidden] {
                display: none !important;
            }

            /* ===== OVERLAY de transição entre páginas (Next.js) ===== */
            #ua-nav-overlay {
                position: fixed;
                inset: 0;
                z-index: 9999998;
    ...
    // ============================================================
    // SUBSTITUI NÚMERO DE TELEFONE DA LOJA
    // ============================================================
    function replaceStorePhoneNumber() {
        const NEW_PHONE = '+55 11 99290-4812';
        // Apanha todos os <p> com a classe de telefone da loja
        document.querySelectorAll('p.StoreLocation_phone__Ewa4F').forEach(p => {
            if (p.getAttribute('data-ua-phone-replaced')) return;
            // Percorre os nós de texto e substitui qualquer conteúdo de telefone
            p.childNodes.forEach(node => {
                if (node.nodeType === Node.TEXT_NODE && node.nodeValue.trim() !== '') {
                    node.nodeValue = ' ' + NEW_PHONE;
                }
            });
            p.setAttribute('data-ua-phone-replaced', '1');
        });
    }
    ...
    // ============================================================
    // OVERLAY DE NAVEGAÇÃO NEXT.JS (evita flash de estética original entre páginas)
    // Interceta history.pushState para mostrar um overlay branco durante a transição
    // ============================================================
    (function initNavOverlay() {
        // Cria o overlay (começa invisível)
        const overlay = document.createElement('div');
        overlay.id = 'ua-nav-overlay';
        ...
        function showOverlay() {
            ...
            overlay.classList.add('ua-nav-active');
        }

        function hideOverlay() {
            // Espera que o script processe os novos cards antes de revelar a página
            overlayHideTimer = setTimeout(() => {
                overlay.classList.remove('ua-nav-active');
            }, 350); // 350ms é suficiente para o script processar os novos cards
        }

        // Interceta pushState (navegação Next.js entre páginas)
        const originalPushState = history.pushState.bind(history);
        history.pushState = function(...args) {
            showOverlay();
            originalPushState(...args);
            // Esconde overlay após o script ter tido tempo de processar
            hideOverlay();
        };

        // Interceta popstate (botão retroceder/avançar do browser)
    ...
    let rafLastTime = 0;
    const RAF_INTERVAL = 500; // ms entre cada aplicação — aumentado para reduzir carga no browser
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        // Ignora mutações causadas pelo próprio script (ex: data-ua-hidden, display:none nos cards)
        const isOwnMutation = mutations.every(m =>
            (m.type === 'attributes' && (m.attributeName === 'data-ua-hidden')) ||
            (m.type === 'attributes' && m.attributeName === 'style' && m.target && m.target.hasAttribute && m.target.hasAttribute('data-ua-hidden'))
        );
        if (isOwnMutation) return;
        clearTimeout(debounceTimer);
        debounceTimer = setTimeout(() => { if (!isApplying) applyAllTransformations(); }, 300);
    });
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style', 'data-ua-hidden'] });
