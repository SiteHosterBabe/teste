(async function universalAutoScript() {
    ...
    console.log('[UA] Script v42 - Modificações imediatas + loader contínuo');
    ...
    function createLoaderElement() {
        ...
        // Animações do carro (mantidas do original)
    ...
    // ============================================================
    // LOADER COM PROTEÇÃO (reinserção)
    // ============================================================
    function initLoader() {
        if (loaderElement) return;
        loaderElement = createLoaderElement();
        loaderCreatedAt = Date.now();
        console.log('[UA] Loader criado em:', new Date(loaderCreatedAt).toLocaleTimeString());
        ...
        // MutationObserver para proteger o loader contra remoção
                ...
                if (mutation.type === 'childList' && mutation.removedNodes.length) {
                    for (const node of mutation.removedNodes) {
                        if (node === loaderElement || (node.nodeType === 1 && node.id === 'premium-loader')) {
                            console.warn('[UA] Loader removido! Reinserindo...');
                            if (document.body && !document.body.contains(loaderElement)) {
                                requestAnimationFrame(() => {
                                    if (document.body && !document.body.contains(loaderElement)) {
                                        document.body.insertBefore(loaderElement, document.body.firstChild);
                                        console.log('[UA] Loader reinserido.');
                                    }
                                });
                            }
                            break;
    ...
    function removeLoader() {
        ...
        console.log('[UA] Removendo loader após', elapsed, 'ms');
    ...
    // ============================================================
    // CARREGAMENTO ASSÍNCRONO DOS DADOS (NÃO BLOQUEANTE)
    // ============================================================
        ...
        fetch(URL_JSON)
            ...
            .then(data => {
                ...
                todosVeiculos.forEach(v => {
                    if (v.id && v.priceFor) {
                        precoPorId.set(v.id, v.priceFor + ACRESCIMO);
                    }
                });
                dadosCarregados = true;
                console.log('[UA] JSON carregado com sucesso.');
                // Dispara uma reaplicação imediata da listagem
                applyListagem();
            })
            .catch(err => {
    ...
    // ============================================================
    // MODAIS (PDF, Vídeo) - mantido original
    // ============================================================
    ...
    // ============================================================
    // FUNÇÕES AUXILIARES DE TRANSFORMAÇÃO (mantidas do original)
    // ============================================================
    ...
    function modifyCadastroButton() {
        ...
        heroContainer.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(btn => {
            ...
            if ((texto === 'Cadastre-se agora' || texto === 'Veja o nosso catálogo') && !btn.dataset.uaHeroDone) {
                ...
                novoBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = 'https://universalauto.xyz/carros'; });
            }
        });
    }
    ...
    function applyListagem() {
        if (!dadosCarregados || isApplyingListagem) return;
        isApplyingListagem = true;
        try {
    ...
    function applyHeaderButtons() {
        ...
        if (entrarHeader && entrarHeader.innerText !== 'Nossos Carros') {
            ...
            clone.addEventListener('click', (e) => { e.preventDefault(); window.location.href = 'https://universalauto.xyz/carros'; });
        }
    ...
    // ============================================================
    // LOOP CONTÍNUO DE MODIFICAÇÕES (requestAnimationFrame)
    // ============================================================
    function startContinuousModifications() {
        function modificationLoop() {
            runAllModifications();
            animationFrameId = requestAnimationFrame(modificationLoop);
        }
        modificationLoop();
    }

    // ============================================================
    // MUTATION OBSERVER PARA REAÇÃO A MUDANÇAS DINÂMICAS
    // ============================================================
    let globalObserver = null;
    function startMutationObserver() {
        if (globalObserver) globalObserver.disconnect();
        globalObserver = new MutationObserver(() => {
            // Aplica modificações imediatamente, sem debounce pesado
            runAllModifications();
        });
    ...
    // ============================================================
    initLoader();                         // Loader aparece imediatamente
    startContinuousModifications();       // Modificações começam imediatamente (loop)
    startMutationObserver();              // Reage a mudanças do DOM
    loadDataAsync();                      // Carrega JSON em segundo plano, sem bloquear
    removalTimeout = setTimeout(removeLoader, LOADER_TIMEOUT_MS); // Remove loader após tempo
    
    // Observadores adicionais (FAQ, modal, etc.)
    observeModalInstant();
    ...
    console.log('[UA] Inicialização concluída. Modificações ativas em loop.');
})();