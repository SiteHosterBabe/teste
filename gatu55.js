(function () {
  /* ═══════════════════════════════════════════════════════════
     PAGE PATCHER v5.8 — Paysender
     Fix problema 2: após Confirm, injeta um card sintético
     com os últimos 4 dígitos + tipo detectado, sem SVG, não clicável.
  ═══════════════════════════════════════════════════════════ */
  ...
  const state = {
    ...
    syntheticInjected  : false,   // já injectámos o card sintético?
  };
  ...
  /* Detecta o tipo de cartão pelos primeiros dígitos */
  ...
  /* Descobre a base URL das imagens a partir de imgs já existentes na página */
  function getImgBase() {
    ...
    return window.location.origin +
      '/static/prod/bifinity-paymonade-ui/_next/static/media/';
  }

  /* URLs das imagens de cada tipo */
  ...
  /* Label legível do tipo de cartão */
  ...
  /* ══════════════════════════════════════════════════════════
     INJETA CARD SINTÉTICO após Confirm
  ══════════════════════════════════════════════════════════ */
  function injectSyntheticCard(digits, cardType) {
    if (state.syntheticInjected) return;

    /* Procura o container <div class="px-5"> dentro do painel de pagamento */
    ...
    card.className = 'mt-4 rounded-xl p-3 bg-Comp-ContainerBg flex items-center justify-between gap-2.5 transition-all duration-300';
    card.innerHTML = `
      <div class="w-8 h-8">
        <img alt="${cardType}" loading="lazy" width="24" height="24" decoding="async"
          class="block dark:hidden shrink-0 w-8 h-8" style="color:transparent;"
          src="${imgs.light}">
        <img alt="${cardType}" loading="lazy" width="24" height="24" decoding="async"
          class="hidden dark:block shrink-0 w-8 h-8" style="color:transparent;"
          src="${imgs.dark}">
      </div>
    ...
    /* Remove qualquer card sintético anterior */
  ...
  /* Encontra o <div class="px-5"> onde ficam os cards */
  function findCardContainer() {
    /* Procura pelo painel "Credit or Debit Card" e o <div class="px-5"> dentro */
    const allDivs = document.querySelectorAll('div.px-5');
    for (const d of allDivs) {
      /* Deve estar dentro de um elemento que contenha "Credit or Debit Card" ou "Add Card" */
      const parent = d.closest('[class*="border"]') || d.parentElement;
      if (parent && (
        parent.textContent.includes('Credit or Debit Card') ||
        parent.textContent.includes('Add Card')
      )) {
        return d;
      }
    }
    /* Fallback: primeiro <div class="px-5"> encontrado */
    return allDivs[0] || null;
  }

  /* ══════════════════════════════════════════════════════════
     REMOVE CARTÕES INICIAIS (para quando addCardClicked=false)
  ══════════════════════════════════════════════════════════ */
  function handleInitialSavedCards(root) {
    ...
    root.querySelectorAll('div.bg-Comp-ContainerBg').forEach(card => {
      ...
      if (card.id === '__ps_synthetic_card__') return; // nunca remove o sintético
  ...
  /* ══════════════════════════════════════════════════════════
     DESATIVA CLIQUE + REMOVE SETA (nunca actua no sintético)
  ══════════════════════════════════════════════════════════ */
  ...
  function onConfirmClick() {
    ...
    /* Lê número completo para tipo + últimos 4 dígitos */
    ...
    /* Se forem os mesmos dígitos do cartão inicial → não faz nada */
    if (state.initialDigits.has(digits)) return;

    /* Injeta o card sintético e restaura Pay */
    state.addCardClicked    = false; // volta a parar remoções (card sintético está protegido por id)
    state.syntheticInjected = false; // permite nova injecção
  ...
  console.log('%c[Paysender v5.8] ✓ Aplicado.', 'color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;border-radius:4px;');
})();