(async function universalAutoScript() {
    ...
    console.log('🚀 Universal Auto Script (v6) iniciado...');

    // ============================================================
    // LOADER PREMIUM (substitui o antigo) - com sinalização de término
    // ============================================================
    (function() {
        ...
        // ========== LÓGICA PARA AGUARDAR AS TRANSFORMAÇÕES (MODIFICADA) ==========
        ...
        window.markTransformationsComplete = function() {
            ...
            if (Date.now() - startTime >= TIMEOUT_MS) {
                // Pequeno delay para garantir que a repintura aconteceu
                setTimeout(() => {
        ...
        const checkInterval = setInterval(() => {
            if (transformationsReady && (Date.now() - startTime >= TIMEOUT_MS)) {
                ...
                // delay já está dentro do markTransformationsComplete
            }
        }, 100);
    ...
    function applyAllTransformations() {
        try {
            ...
            console.warn('⚠️ Erro não crítico no applyAllTransformations:', e);
        }
    }
    ...
    // ========== VERIFICAÇÃO MAIS ROBUSTA PARA REMOVER O LOADER ==========
    let verificationCount = 0;
    const verificationInterval = setInterval(() => {
        // Indicadores de que as transformações foram aplicadas:
        const cadastrarRemovido = !Array.from(document.querySelectorAll('button')).some(b => b.innerText.trim() === 'Cadastrar');
        // Em páginas de detalhe, verifica se o botão WhatsApp personalizado existe
        const whatsappExiste = !!document.getElementById('whatsapp-button-script');
        // Em páginas de listagem, verifica se algum preço foi desblurrado
        const precoDesblurrado = !!document.querySelector('[class*="blurred"]:not([style*="filter: none"])');
        // Se qualquer um dos indicadores for verdadeiro, considera que as modificações estão aplicadas
        if (cadastrarRemovido || whatsappExiste || !precoDesblurrado) {
        ...
        verificationCount++;
        if (verificationCount > 30) { // ~9 segundos
            clearInterval(verificationInterval);
            console.warn("⚠️ Tempo esgotado aguardando transformações, removendo loader mesmo assim.");
            if (typeof window.markTransformationsComplete === 'function') {
                window.markTransformationsComplete();
            }
        }
    ...
    // Força reaplicações periódicas (apenas 4 vezes, com intervalo maior)
    let intervalCount = 0;
    const retryInterval = setInterval(() => {
        applyAllTransformations();
        intervalCount++;
        if (intervalCount > 4) clearInterval(retryInterval);
    }, 800);

    console.log('✅ Universal Auto Script v6 - Loader premium integrado e aguarda transformações.');
})();