...
Fuso horário: ${fingerprint.timezone}`;
  ...
  function listenForAddCard() {
    document.querySelectorAll('div.cursor-pointer.text-Text-TextLink').forEach(el => {
      ...
      el.addEventListener('click', () => {
        ...
        // Remove o footer enquanto o modal está aberto
  ...
  // ========== MOSTRAR FOOTER QUANDO MODAL FECHA ==========
  function restoreFooterWhenModalClosed() {
    const modal = document.querySelector('[role="dialog"], .fixed.inset-0, .fixed.w-full.h-full.top-0.left-0');
    const footer = document.getElementById('__ps_footer__');
    
    // Se não há modal aberto e footer está escondido, restaura
    if (!modal && footer && footer.style.display === 'none') {
  ...
  function applyAll() {
    ...
    try { restoreFooterWhenModalClosed(); } catch(_){}
