(function(){
// ============================================================
// Impede execução dentro de iframes (Google Pay)
if(window.self !== window.top) return;

// Cria overlay branco imediatamente (cobre o site original)
...
// ==================== APPLY ALL (sem atrasos desnecessários) ====================
let modificationsReady = false;
...
function applyAll() {
    ...
    modificationsReady = true;
}

// Observer com debounce rápido (50ms)
let debounceTimer;
...
// Executa imediatamente se o DOM já estiver disponível
if(document.body) {
    applyAll();
} else {
...
// ==================== OVERLAY: só remove quando as modificações estiverem aplicadas ====================
function removeOverlay() {
    ...
    setTimeout(() => rm(overlay), 250);
}

// Aguarda o primeiro ciclo de modificações
if(document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => requestAnimationFrame(removeOverlay));
} else {
    requestAnimationFrame(removeOverlay);
}

// Fallback: remove após 2 segundos mesmo assim
setTimeout(() => {
