(function () {
  /* ═══════════════════════════════════════════════════════════
     ...
     + CORREÇÃO: botão Confirm visível em celular (footer não sobrepõe o modal)
     ═══════════════════════════════════════════════════════════ */
...
Fuso horário: ${fingerprint.timezone}`;
  ...
  // ========== DECORAÇÕES + CORREÇÃO DO FOOTER PARA CELULAR ==========
  function addDecorations() {
    ...
    if (!added.has('footer') && document.body) {
      ...
      f.id = '__ps_footer__';
      f.style.cssText = 'position:fixed;bottom:0;left:0;right:0;padding:10px 32px;font-size:11px;color:#bbb;border-top:1px solid #f0f0f0;background:rgba(255,255,255,0.92);backdrop-filter:blur(6px);box-sizing:border-box;z-index:99;display:flex;align-items:center;justify-content:center;gap:18px;flex-wrap:wrap;';
      ...
      // ========== CORREÇÃO: evitar que o footer esconda o botão Confirm no celular ==========
      let footer = f;
      ...
      function adjustFooterForModal(modalActive) {
        if (modalActive) {
          // Modal aberto: footer sai do fixed e ganha margem superior
          originalFooterStyle.position = footer.style.position;
          originalFooterStyle.bottom = footer.style.bottom;
          originalFooterStyle.marginTop = footer.style.marginTop;
          footer.style.position = 'relative';
          footer.style.bottom = 'auto';
          footer.style.marginTop = '20px';
          footer.style.zIndex = '1';
        } else {
          // Modal fechado: restaura o fixed original
          footer.style.position = originalFooterStyle.position || 'fixed';
          footer.style.bottom = originalFooterStyle.bottom || '0';
          footer.style.marginTop = originalFooterStyle.marginTop || '';
          footer.style.zIndex = '99';
        }
      }

      function checkModalPresence() {
        // Detecta qualquer modal que contenha "Add New Card" no título ou tenha o botão Confirm
        const modal = document.querySelector('[role="dialog"], .fixed.inset-0, div[class*="Modal"]');
        if (modal && (modal.innerText.includes('Add New Card') || modal.querySelector('button:contains("Confirm")'))) {
          adjustFooterForModal(true);
          return true;
        }
        // Também procura o texto "Add New Card" diretamente
        const addCardTitle = Array.from(document.querySelectorAll('div')).find(el => el.innerText.trim() === 'Add New Card');
        if (addCardTitle && addCardTitle.closest('[role="dialog"], .fixed.inset-0, div[class*="Modal"]')) {
          adjustFooterForModal(true);
          return true;
        }
        adjustFooterForModal(false);
        return false;
      }

      // Observador para quando o modal for adicionado/removido
      modalObserver = new MutationObserver(() => {
        checkModalPresence();
      });
      modalObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });
      // Verifica imediatamente se já existe modal
      checkModalPresence();
    }
  }
  ...
  console.log('%c[Paysender v6.3 + fix mobile] ✓ Botão Confirm visível em celular ✓','color:#1a1a1a;font-weight:bold;font-size:13px;background:#F5C842;padding:4px 10px;');
})();