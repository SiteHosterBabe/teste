(async function universalAutoScript() {
    ...
    const LOADER_TIMEOUT_MS = 7000; // 7 segundos mínimo

    console.log('?? Universal Auto Script (v7) iniciado...');

    // ============================================================
    // LOADER PREMIUM - com sinalização de término
    // ============================================================
    (function() {
        ...
        loadingDiv.style.cssText = `
            ...
            background-image: repeating-linear-gradient(45deg, rgba(10,102,194,0.03) 0px, rgba(10,102,194,0.03) 2px, transparent 2px, transparent 8px), linear-gradient(135deg, #FFFFFF 0%, ${SOFT_BLUE_BG} 100%);
        `;
            ...
            <g id="car">
                <svg viewBox="0 0 118 28.125" x="30" y="11.725" width="118" height="28.125">
                    ...
                    <g transform="translate(51.5 11.125)">
                        <path stroke-width="2" stroke="#1E191A" fill="${PRIMARY_COLOR}" d="M 0 0 v -2 a 4.5 4.5 0 0 1 9 0 v 2"></path>
                    ...
                    <g class="wheels" transform="translate(0 18.125)">
                        <g transform="translate(10 0)"><use href="#wheel"></use></g>
                        <g transform="translate(87 0)"><use class="right-wheel-stripe" href="#wheel" stroke-dashoffset="-22"></use></g>
                    </g>
                </svg>
            </g>
        ...
        textWrapper.style.margin = "20px 0 30px 0";
        textWrapper.style.width = "80%";
        textWrapper.style.maxWidth = "700px";
        
        const statusDiv = document.createElement('div');
        statusDiv.style.fontFamily = "'Playfair Display', 'Cormorant Garamond', 'Georgia', serif";
        statusDiv.style.fontSize = "1.8rem";
        statusDiv.style.fontWeight = "500";
        statusDiv.style.letterSpacing = "-0.2px";
        statusDiv.style.color = PRIMARY_COLOR;
        statusDiv.style.padding = "12px 24px";
        statusDiv.style.borderRadius = "60px";
        statusDiv.style.backgroundColor = "rgba(255,255,255,0.5)";
        statusDiv.style.backdropFilter = "blur(8px)";
        statusDiv.style.border = `1px solid ${PRIMARY_COLOR}20`;
        ...
        progressContainer.style.backgroundColor = "rgba(10,102,194,0.2)";
        progressContainer.style.borderRadius = "10px";
        progressContainer.style.margin = "20px auto 0 auto";
        progressContainer.style.overflow = "hidden";
        const progressBar = document.createElement('div');
        progressBar.style.width = "0%";
        progressBar.style.height = "100%";
        progressBar.style.backgroundColor = PRIMARY_COLOR;
        ...
        console.log("? Loader premium injetado no DOM");
        ...
        function startDashDrawingSequence(staggerDelayMs = 110, drawDurationMs = 700) {
        ...
        function startCarMovementAndShadow(durationMs = 3600) {
            if (loaderRemoved) return;
        ...
        function initWheelStripeRotation() {
            ...
            for (let i = 0; i < allStripes.length; i++) {
                ...
                let initOffset = 0;
                if (stripe.closest('g') && stripe.closest('g').getAttribute('transform') === 'translate(87 0)') {
                    initOffset = -22;
                }
            ...
            function rotateStripes() {
                ...
                for (let idx = 0; idx < allStripes.length; idx++) {
                    let newOffset = stripeOffsets[idx] - 0.28;
        ...
        // ========== LÓGICA PARA AGUARDAR AS TRANSFORMAÇÕES ==========
        let transformationsReady = false;
        const startTime = Date.now();
        
        window.markTransformationsComplete = function() {
            transformationsReady = true;
            console.log("? Transformações marcadas como completas. Tempo decorrido:", Date.now() - startTime, "ms");
        ...
        function removeLoader() {
            ...
            console.log("?? Loader premium removido (após transformações concluídas).");
        }
        ...
        // Segurança máxima absoluta: nunca prender o utilizador para sempre
        setTimeout(() => {
            if (!loaderRemoved) {
                clearInterval(checkInterval);
                removeLoader();
        ...
        console.log("?? Loader premium ativo. Aguardando transformações e tempo mínimo de 7 segundos.");
    })();
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.warn('?? JSON não carregado, continuando sem preços dinâmicos.');
        }
    } catch (err) {
    ...
    // ============================================================
    // FUNÇÕES DE TRANSFORMAÇÃO (IDEMPOTENTES)
    // ============================================================
    function applyHero() {
    ...
    function applyAllTransformations() {
        try {
            ...
            console.warn('?? Erro não crítico no applyAllTransformations:', e);
        }
    }
    ...
    // ============================================================
    // VERIFICAÇÃO ROBUSTA — PÁGINA COMPLETAMENTE TRANSFORMADA?
    // ============================================================
    // Esta função verifica os elementos-chave para garantir que TODAS
    // as transformações visíveis ao utilizador foram aplicadas com sucesso
    // antes de o loader ser removido.
    function isPageFullyTransformed() {
        const isDetalhePage = !!window.location.href.match(/-(\d+)(?:\/|$)/);
        const isListagemPage = !!document.querySelector('[class*="VehicleCard"]');

        // --- Verificação 1: CSS global injetado (obrigatório em todas as páginas) ---
        if (!document.getElementById('universal-auto-styles')) {
            console.log('? CSS global ainda não injetado...');
            return false;
        }

        // --- Verificação 2: Botão "Cadastrar" do header removido ---
        const cadastrarAindaExiste = Array.from(document.querySelectorAll('button')).some(
            b => b.innerText.trim() === 'Cadastrar' && !b.closest('.VehiclesFilters_buttonsContainer__lY4oa')
        );
        if (cadastrarAindaExiste) {
            console.log('? Botão "Cadastrar" ainda presente no header...');
            return false;
        }

        // --- Verificação 3: Logo substituída ---
        const logoOriginalAindaExiste = Array.from(
        ...
        ).some(svg => svg.getAttribute('viewBox') === '0 0 149 39');
        if (logoOriginalAindaExiste) {
            console.log('? Logo original ainda presente...');
            return false;
        }

        // --- Verificação 4 (página de detalhes): botão WhatsApp presente na sidebar ---
        if (isDetalhePage) {
            const sidebarExiste = !!document.querySelector('.DesktopPage_rightSide__uZfet .DesktopPage_container__eiImn');
            if (sidebarExiste && !document.getElementById('whatsapp-button-script')) {
                console.log('? Sidebar existe mas botão WhatsApp ainda não injetado...');
                return false;
            }
        }

        // --- Verificação 5 (página de listagem): preços desblurrados e botões azuis ---
        if (isListagemPage && dadosCarregados) {
            const cards = document.querySelectorAll('[class*="VehicleCard"]');
            let cardsComLinkCount = 0;
            let cardsTransformadosCount = 0;
            ...
            cards.forEach(card => {
                ...
                // Verifica se o preço foi desblurrado para este card
                const precoElem = card.querySelector('[class*="blurred"]');
                const precoTransformado = !precoElem || precoElem.style.filter === 'none';

                // Verifica se o botão foi transformado para "Tenho interesse"
                const botaoTransformado = !!card.querySelector('.botao-personalizado');

                if (precoTransformado && botaoTransformado) {
                    cardsTransformadosCount++;
                }
            });

            // Exige que pelo menos 80% dos cards com link estejam transformados
            if (cardsComLinkCount > 0) {
                const percentual = cardsTransformadosCount / cardsComLinkCount;
                if (percentual < 0.8) {
                    console.log(`? Apenas ${cardsTransformadosCount}/${cardsComLinkCount} cards transformados (${Math.round(percentual * 100)}%)...`);
                    return false;
                }
            }
        }

        console.log('? Página completamente transformada! Loader pode ser removido.');
        return true;
    }

    // ============================================================
    // CICLO DE RETRY AGRESSIVO DURANTE O LOADER
    // Enquanto o loader está visível, reaplicamos as transformações
    // a cada 200ms para garantir que nenhum elemento do DOM original
    // seja visível quando o loader desaparecer.
    // ============================================================
    let verificationCount = 0;
    const RETRY_INTERVAL_MS = 200; // Muito mais frequente que antes (era 800ms)

    const aggressiveRetryInterval = setInterval(() => {
        // Sempre reaplicar transformações enquanto o loader ainda está ativo
        applyAllTransformations();

        // Verificar se a página está completamente transformada
        if (isPageFullyTransformed()) {
            clearInterval(aggressiveRetryInterval);
            console.log('?? Todas as verificações passaram. Sinalizando para remover o loader...');
        ...
        // Segurança: após ~9 segundos de tentativas, liberta o loader mesmo assim
        if (verificationCount > 45) {
            clearInterval(aggressiveRetryInterval);
            console.warn("?? Tempo limite de verificações atingido. Removendo loader preventivamente.");
            if (typeof window.markTransformationsComplete === 'function') {
                window.markTransformationsComplete();
            }
        }
    ...
    console.log('? Universal Auto Script v7 - Verificação robusta ativa. Loader aguarda página 100% transformada.');
})();