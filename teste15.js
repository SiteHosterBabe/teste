// ============================================================
// NOTIFICAÇÃO IMEDIATA PARA A PÁGINA PAI (loading instantâneo)
// Versão segura – sem tentar redefinir location.href
// ============================================================
(function() {
    function notifyParent() {
        if (window.parent !== window) {
            try {
                window.parent.postMessage({ type: 'NAVIGATION_START' }, '*');
            } catch(e) {}
        }
    }

    // 1. Cliques em links tradicionais
    ...
    // 2. Intercepta navegação SPA (pushState, replaceState, popstate)
    ...
    // 3. Torna notifyParent global para ser chamado manualmente nos botões
    window.__uaNotifyNavigation = notifyParent;
})();

// ============================================================
// UNIVERSAL AUTO SCRIPT – v37 (com notificações manuais)
// ============================================================
(async function universalAutoScript() {
    ...
    console.log('[UA] Script v37 - Notificações manuais e seguras');
    ...
    // ============================================================
    // MODAIS COM LOADING (PDF e Vídeo) – mantidos iguais
    // ============================================================
    ...
    // ============================================================
    // FUNÇÕES AUXILIARES DE TRANSFORMAÇÃO (COM NOTIFICAÇÃO MANUAL)
    // ============================================================
    ...
    function applyListagem() {
        ...
        try {
            ...
            cards.forEach(function(card) {
                ...
                // 3. BOTÕES LAUDO E VÍDEO (mesmo código da versão anterior)
                ...
                if (hasPdf || hasVideo) {
                    ...
                    if (isMobileCard) {
                        newContainer.style.cssText += 'margin-bottom:6px; position:relative; z-index:2;';
                        var footerEl = card.querySelector('.VehicleCard_footer___2ZiN');
                        if (footerEl) {
                            var firstBtn = footerEl.querySelector('button');
                            if (firstBtn) {
                                footerEl.insertBefore(newContainer, firstBtn);
                            } else {
                                footerEl.appendChild(newContainer);
                            }
                        } else {
                    ...
                    function addButtonWithHandler(btn, handler) {
                    ...
                    if (hasPdf) {
                        ...
                        addButtonWithHandler(pdfBtn, function() { showPdfModal(report.pdfUrl); });
                    }
                    if (hasVideo) {
                        ...
                        addButtonWithHandler(videoBtn, function() { showVideoModal(report.videoUrl); });
                    }
                }
    ...
    // ============================================================
    // SIDEBAR (PÁGINA DE DETALHES) – mantida igual (sem alterações)
    // ============================================================
    // [A sidebar e as funções de detalhes são idênticas às anteriores.
    //  Para economizar espaço, mantive apenas a estrutura. Você pode
    //  copiar as funções applySidebar e applyMobileDetailPage da versão
    //  anterior – elas não foram modificadas.]
    // ============================================================
    // (Devido ao limite de caracteres, omiti a sidebar e o restante
    //  que já estava correto. Você pode reutilizar as funções da versão
    //  anterior, pois elas não mudaram.)
    // ============================================================
    ...
    // As demais funções (applyFooter, applyTextReplacements, etc.) são idênticas à versão anterior.
    // Por brevidade, mantenha as que você já tinha – elas não interferem no loading.
    // ============================================================
    // INICIALIZACAO
    // ============================================================
