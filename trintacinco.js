(async function universalAutoScript() {
    ...
    // Mensagem base do WhatsApp (será acrescida da URL atual)
    const WHATSAPP_BASE_MESSAGE = 'Olá! Tenho interesse em um veículo.';

    console.log('[UA] Universal Auto Script (v23 - Mobile Flash Fix) iniciado...');

    // ============================================================
    // LOADER PREMIUM (mantido exatamente igual ao seu v15)
    // ============================================================
    (function() {
        ...
        // A remoção do loader será feita fora desta IIFE pelo safeRemoveLoader
        // Por isso, não chamamos removeLoader aqui.
    })();
    ...
    function modifyCadastroButton() {
        ...
        botoes.forEach(function(btn) {
            ...
            if ((texto === 'Cadastre-se agora' || texto === 'Veja o nosso catálogo') && !btn.dataset.uaHeroDone) {
                ...
                console.log('[UA] Botão hero → "Veja o nosso catálogo" (/carros).');
            }
        });
    }
    ...
    function modifyFaqCadastroButton() {
        ...
        faqButtons.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(function(btn) {
            ...
            if (span.innerText.trim() === 'Cadastre-se agora' && !btn.dataset.uaFaqDone) {
                ...
                console.log('[UA] Botão FAQ → "Nosso Contacto" (mailto).');
            }
        });
    }
    ...
    // ============================================================
    // NOVA FUNÇÃO: Substitui o título da secção "How it works"
    // ============================================================
    function modifyHowItWorksTitle() {
        ...
        if (titleElement) {
            ...
            if (titleElement.innerHTML !== newHTML) {
                ...
                console.log('[UA] Título da secção "How it works" substituído.');
            }
        }
    }

    // ============================================================
    // SUBSTITUIÇÕES DO ACORDEÃO FAQ (TÍTULOS E RESPOSTAS - SEM FLASH)
    // ============================================================
        ...
        titles.forEach(span => {
            ...
            for (let item of titulosMapping) {
                if (current === item.original && span.innerText !== item.novo) {
                    ...
                    replacedCount++;
                    console.log(`[UA] Título FAQ substituído: "${current}" → "${item.novo}"`);
                    break;
                }
            }
        });
        if (replacedCount === 0 && titles.length > 0) {
            console.warn('[UA] Nenhum título FAQ foi substituído. Verifique os textos originais.');
        }
            ...
            for (let item of respostasMapping) {
                if (current === item.original || current.startsWith(item.original)) {
                    if (p.innerText !== item.novo) {
                        ...
                        replacedCount++;
                        console.log(`[UA] Resposta FAQ substituída: "${current.substring(0, 50)}..."`);
                    }
                    break;
                }
            }
        });
        if (replacedCount === 0 && paragraphs.length > 0) {
            console.warn('[UA] Nenhuma resposta FAQ foi substituída. Verifique os textos originais.');
        }
        return replacedCount;
    }
    ...
    // ============================================================
    // OBSERVADORES (sem flash e permanentes)
    // ============================================================
    function observeAccordionExpansion() {
        const observer = new MutationObserver(mutations => {
            mutations.forEach(mut => {
                ...
                console.log('[UA] Acordeão aberto — conteúdo ocultado, textos substituídos, reexibido.');
            });
        });
        ...
        console.log('[UA] Observer de abertura do acordeão activo (sem flash).');
    }
    ...
    function watchFaqPermanently() {
        ...
        console.log('[UA] Observer permanente de títulos e respostas FAQ activo.');
    }
    ...
    // Alvos: .DisplayContent_hideDesktop__hlDJO com SVG 39x39
    // ============================================================
    function applyMobileNavLogo() {
        ...
        mobileLogoDivs.forEach(function(div) {
            ...
            // Redireciona o link pai para universalauto.xyz
            ...
            if (parentLink) {
                ...
                // Remove listener original clonando o elemento
                ...
                // Agora aponta div para o novo link clonado
            ...
            console.log('[UA] Logo mobile substituído.');
        });
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 2: Substitui secção de interesse mobile
    // Alvo: .MobilePage_contentInterest__dY5dv — botão WhatsApp + preço
    // ============================================================
    function applyMobileInterestSection() {
        ...
        interestDivs.forEach(function(div) {
            ...
            console.log('[UA] Secção de interesse mobile substituída com WhatsApp' + (precoHtml ? ' e preço.' : '.'));
        });
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 3: Substitui imagem hero mobile
    // Alvo: img.Hero_heroImageMobile__TXyFn com src computer-mobile.webp
    // ============================================================
    function applyMobileHeroImage() {
        ...
        if (src.includes('computer-mobile.webp') || src === '/home/computer-mobile.webp') {
            ...
            console.log('[UA] Imagem hero mobile substituída.');
        }
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 4: Remove botão hamburger menu
    // Alvo: button[aria-label="abrir menu"]
    // ============================================================
    function removeHamburgerMenu() {
        var hamburger = document.querySelector('button[aria-label="abrir menu"]');
        if (hamburger) {
            hamburger.remove();
            console.log('[UA] Botão hamburger removido.');
        }
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 5: Remove botão "Entrar" dentro do modal de filtros
    // Alvo: .VehiclesFilters_buttonsContainer__lY4oa (dentro do modal mobile de filtros)
    // ============================================================
        ...
        containers.forEach(function(container) {
            container.querySelectorAll('button').forEach(function(btn) {
                ...
                if (txt === 'Entrar' || txt === 'Cadastrar') {
                    btn.remove();
                    console.log('[UA] Botão "' + txt + '" removido do modal de filtros mobile.');
                }
            });
        });
    }
    ...
    // ============================================================
    // [MOBILE] OBSERVER: Detecta abertura do modal de filtros e
    // remove imediatamente o botão "Entrar" antes de ser visível
    // ============================================================
        ...
        var obs = new MutationObserver(function(mutations) {
            mutations.forEach(function(mut) {
                ...
                mut.addedNodes.forEach(function(node) {
                    ...
                    // Verifica se o nó adicionado É ou CONTÉM o container de botões do filtro
                    if (
                        (node.classList && node.classList.contains('VehiclesFilters_buttonsContainer__lY4oa')) ||
                        (node.querySelector && node.querySelector('.VehiclesFilters_buttonsContainer__lY4oa'))
                    ) {
                        // Ocultamos imediatamente para não haver flash
                        var containers = node.classList && node.classList.contains('VehiclesFilters_buttonsContainer__lY4oa')
                            ? [node]
                            : Array.from(node.querySelectorAll('.VehiclesFilters_buttonsContainer__lY4oa'));
                        containers.forEach(function(c) {
                            c.style.setProperty('visibility', 'hidden', 'important');
                        });
                        // Removemos e reexibimos no próximo frame
    ...
    // ============================================================
    // [MOBILE] EXECUÇÃO ANTECIPADA (elimina o flash)
    // ============================================================
    function runMobileTransformationsEarly() {
    ...
    function applyAllTransformations() {
        try {
            ...
            // ── MOBILE (já chamadas também antecipadamente, mas reforça) ──
            applyMobileNavLogo();
            applyMobileHeroImage();
            applyMobileInterestSection();
            removeHamburgerMenu();
    ...
    // Executa transformações iniciais
    applyAllTransformations();
    runMobileTransformationsEarly();

    // ============================================================
    // EVENT LISTENER GLOBAL PARA WHATSAPP (SEM ALTERAR O BOTÃO)
    // ============================================================
    document.body.addEventListener('click', function(e) {
        ...
        if (whatsappBtn) {
            ...
            console.log('[UA] WhatsApp clicado – mensagem com URL enviada.');
        }
    });

    // ============================================================
    // VERIFICACAO ROBUSTA COM REMOÇÃO SEGURA (sem flash)
    // ============================================================
    function isPageFullyTransformed() {
        ...
        // [MOBILE] Verifica imagem hero mobile
        var mobileHero = document.querySelector('img.Hero_heroImageMobile__TXyFn');
        if (mobileHero) {
        ...
        // [MOBILE] Verifica logo mobile ainda com SVG original
        var mobileLogoSvgExiste = Array.from(
            document.querySelectorAll('.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO svg')
        ).some(function(s) { return s.getAttribute('viewBox') === '0 0 39 39'; });
        if (mobileLogoSvgExiste) return false;

        // [MOBILE] Verifica se hamburger ainda existe
        if (document.querySelector('button[aria-label="abrir menu"]')) return false;

        // [MOBILE] Verifica se botão "Entrar" ainda existe no container de filtros
        ...
        if (isDetalhePage) {
            ...
            // [MOBILE] Verifica secção de interesse mobile
            var mobileInterest = document.querySelector('.MobilePage_contentInterest__dY5dv');
            if (mobileInterest && !mobileInterest.querySelector('.whatsapp-button-custom')) return false;
        }
    ...
    // ============================================================
    // [FIX MOBILE] Remove loader APENAS quando tudo estiver transformado
    // ============================================================
    let loaderRemovedGlobal = false;

    function safeRemoveLoader() {
        if (loaderRemovedGlobal) return;
        
        // 1. Aplica TODAS as transformações uma última vez (forçado)
        applyAllTransformations();
        
        // 2. Verifica se realmente ficou tudo ok
        if (!isPageFullyTransformed()) {
            console.warn('[UA] Transformações incompletas, aguardando mais 200ms...');
            setTimeout(safeRemoveLoader, 200);
            return;
        }
        
        // 3. Usa requestAnimationFrame para garantir que o browser já processou o DOM
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                const mask = document.getElementById('ua-page-mask');
                ...
                console.log('[UA] Loader e máscara removidos APÓS transformações completas.');
    ...
    var aggressiveRetryInterval = setInterval(function() {
        ...
        verificationCount++;
        if (verificationCount > 60) {  // 12 segundos (60 * 200ms)
            clearInterval(aggressiveRetryInterval);
            console.warn('[UA] Limite de verificacoes atingido. Removendo loader mesmo assim.');
            safeRemoveLoader();
        }
    }, 200);

    console.log('[UA] Script v23 (Mobile Flash Fix) ativo.');
})();