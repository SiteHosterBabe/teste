(async function universalAutoScript() {
    ...
    console.log('[UA] Universal Auto Script (v12 - FAQ e contactos) iniciado...');

    // ============================================================
    // LOADER PREMIUM (mantido integralmente, omitido por brevidade)
    // ============================================================
    (function() {
        ...
        // ========== FUNCAO REMOVELOADER COM FADE OUT ==========
    ...
    function modifySeparatorsBackground() {
        ...
        if (leftBorders.length || rightBorders.length) console.log('[UA] Backgrounds dos separadores alterados.');
    }
        ...
        botoes.forEach(function(btn) {
            ...
            if (span && span.innerText.trim() === 'Cadatre-se agora') {
                if (span.innerText !== 'Nosso Catálogo') {
                    ...
                    console.log('[UA] Botão "Cadatre-se agora" transformado em "Nosso Catálogo".');
                }
            }
        });
    }
    ...
    function replaceHowItWorksTexts() {
        ...
        steps.forEach(function(step) {
            var h2Elements = document.querySelectorAll('h2.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-md__QVgP4');
            h2Elements.forEach(function(h2) {
                if (h2.innerText.trim() === step.h2Text && h2.innerText !== step.newH2) {
                    h2.innerText = step.newH2;
                    console.log('[UA] H2 substituído:', step.h2Text, '->', step.newH2);
                }
            });
            var pElements = document.querySelectorAll('p.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--body-md__KI9TK');
            pElements.forEach(function(p) {
                if (p.innerText.trim() === step.pText && p.innerText !== step.newP) {
                    p.innerText = step.newP;
                    console.log('[UA] P substituído.');
                }
            });
        });
    }
    ...
    function modifyEquipeEspecializadaBackground() {
        ...
        if (div) {
            var targetGradient = 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)';
            var computedBg = window.getComputedStyle(div).background;
            if (!computedBg.includes(targetGradient)) {
                div.style.background = targetGradient;
                div.style.setProperty('background', targetGradient, 'important');
                console.log('[UA] Background da seção Equipe Especializada alterado.');
            }
        }
    }
    ...
    function modifyFAQSection() {
        ...
        if (!faqDiv) return;
        var targetGradient = 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)';
        var computedBg = window.getComputedStyle(faqDiv).background;
        if (!computedBg.includes(targetGradient)) {
        ...
        if (h2 && h2.innerText.trim() === 'Não possui CNPJ e quer um Seminovo Localiza?') {
            if (h2.innerText !== 'Gostaria de conhecer as viaturas disponíveis em exposição?') {
                h2.innerText = 'Gostaria de conhecer as viaturas disponíveis em exposição?';
                console.log('[UA] Título FAQ substituído.');
            }
        }
        ...
        if (p && p.innerText.trim() === 'O Universal Auto é de uso exclusivo de empresas revendedoras de carros. Se você é uma pessoa física e quer comprar um Seminovo Localiza, acesse nosso estoque pelo link abaixo!') {
            if (p.innerText !== 'O Universal Auto possui loja fisica em São Paulo. Se você quer visitar nossa loja fisica e comprar um carro da universal, acesse nosso estoque em exposição pelo link abaixo!') {
                ...
                console.log('[UA] Parágrafo FAQ substituído.');
            }
        }
        ...
        if (linkP && linkP.innerText.trim() === 'https://seminovos.localiza.com/') {
            if (linkP.innerText !== 'https://universalautorepasse.com.br/') {
                linkP.innerText = 'https://universalautorepasse.com.br/';
                console.log('[UA] Link FAQ substituído.');
            }
        }
    ...
    function modifyLoginDialog() {
        ...
        if (p && p.innerText.trim() === 'Faça o Login ou cadastre-se para ter acesso a todas nossas ofertas') {
            if (p.innerText !== 'Consulte o nosso catálogo e utilize os filtros ou a barra de pesquisa para localizar as melhores oportunidades desta marca.') {
                ...
                console.log('[UA] Texto do diálogo substituído.');
            }
        }
        
        var loginBtn = content.querySelector('button.LdsButton-module_lds-button--basic-primary__Nv8Ii');
        if (loginBtn && loginBtn.innerText.includes('Já possui conta? Faça o login')) {
            loginBtn.remove();
            console.log('[UA] Botão de login removido do diálogo.');
        }
        ...
        if (cadastroBtn) {
            ...
            if (span && span.innerText.trim() === 'Cadastre-se') {
                if (span.innerText !== 'Preço Único') {
                    ...
                    console.log('[UA] Botão Cadastre-se transformado em Preço Único com redirecionamento.');
                }
            }
        }
    }
    ...
    function modifyContatoButton() {
        // Botão "Cadastre-se agora" que não foi modificado anteriormente (ex: fora do contexto do catálogo)
        var botoes = document.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk');
        botoes.forEach(function(btn) {
            var span = btn.querySelector('span');
            if (span && span.innerText.trim() === 'Cadastre-se agora') {
                // Só modifica se ainda não foi modificado (evitar duplicar)
                if (span.innerText !== 'Nosso Contacto') {
                    ...
                    console.log('[UA] Botão "Cadastre-se agora" transformado em "Nosso Contacto" com email.');
                }
            }
        });
    }
    ...
    function modifyAccordionFaq() {
        // 1. "Qual é o benefício de realizar o cadastro no portal do lojista?"
        var faq1 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--header__F04Mr .LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq1 && faq1.innerText.trim() === 'Qual é o benefício de realizar o cadastro no portal do lojista?') {
            if (faq1.innerText !== 'O que significa comprar um carro de repasse?') {
                faq1.innerText = 'O que significa comprar um carro de repasse?';
                console.log('[UA] FAQ1 substituído.');
            }
        }
        // Parágrafo correspondente
        var p1 = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (p1 && p1.innerText.trim() === 'Após realizar o cadastro, você passará a ter acesso completo ao catálogo de veículos disponíveis para venda com condições diferenciadas e promoções exclusivas.') {
            if (p1.innerText !== 'Comprar um carro de repasse significa adquirir um veículo no estado em que ele se encontra, geralmente proveniente de trocas em grandes concessionárias. Esses carros são vendidos por valores significativamente abaixo da tabela FIPE porque a loja não realiza revisões estéticas ou mecânicas completas antes da venda, repassando a margem de lucro e a responsabilidade de manutenção para o comprador.') {
                p1.innerText = 'Comprar um carro de repasse significa adquirir um veículo no estado em que ele se encontra, geralmente proveniente de trocas em grandes concessionárias. Esses carros são vendidos por valores significativamente abaixo da tabela FIPE porque a loja não realiza revisões estéticas ou mecânicas completas antes da venda, repassando a margem de lucro e a responsabilidade de manutenção para o comprador.';
                console.log('[UA] Parágrafo FAQ1 substituído.');
            }
        }

        // 2. "Posso realizar o cadastro de empresas de qualquer segmento?"
        var faq2 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--header__F04Mr .LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq2 && faq2.innerText.trim() === 'Posso realizar o cadastro de empresas de qualquer segmento?') {
            if (faq2.innerText !== 'Os veículos possuem garantia de motor e câmbio?') {
                faq2.innerText = 'Os veículos possuem garantia de motor e câmbio?';
                console.log('[UA] FAQ2 substituído.');
            }
        }
        var p2 = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (p2 && p2.innerText.trim() === 'Você poderá cadastrar apenas empresas que possuam um CNAE principal voltado ao segmento de veículos.') {
            if (p2.innerText !== 'Nesta modalidade específica de negócio, os veículos são vendidos sem garantia de mecânica ou estética. O comprador assume o carro "no estado", ciente de que o preço reduzido compensa eventuais manutenções que precisem ser feitas. Por isso, recomendamos sempre levar um mecânico de confiança para avaliar o veículo no pátio antes de fechar o negócio.') {
                p2.innerText = 'Nesta modalidade específica de negócio, os veículos são vendidos sem garantia de mecânica ou estética. O comprador assume o carro "no estado", ciente de que o preço reduzido compensa eventuais manutenções que precisem ser feitas. Por isso, recomendamos sempre levar um mecânico de confiança para avaliar o veículo no pátio antes de fechar o negócio.';
                console.log('[UA] Parágrafo FAQ2 substituído.');
            }
        }

        // 3. "Como posso ter acesso às ofertas disponíveis?"
        var faq3 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--header__F04Mr .LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq3 && faq3.innerText.trim() === 'Como posso ter acesso às ofertas disponíveis?') {
            if (faq3.innerText !== 'É possível financiar um carro de repasse na loja?') {
                faq3.innerText = 'É possível financiar um carro de repasse na loja?';
                console.log('[UA] FAQ3 substituído.');
            }
        }
        var p3 = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (p3 && p3.innerText.trim() === 'Para acessar as ofertas disponíveis é necessário que você realize o cadastro da sua empresa na plataforma.') {
            if (p3.innerText !== 'Sim, a Universal Auto Repasse costuma trabalhar com parcerias bancárias para facilitar o pagamento. No entanto, por serem carros com preços promocionais, as condições de financiamento e a aprovação de crédito dependem do ano do veículo e do perfil do CPF do cliente. Também é comum aceitarem cartões de crédito para parcelamento da entrada ou do valor total.') {
                p3.innerText = 'Sim, a Universal Auto Repasse costuma trabalhar com parcerias bancárias para facilitar o pagamento. No entanto, por serem carros com preços promocionais, as condições de financiamento e a aprovação de crédito dependem do ano do veículo e do perfil do CPF do cliente. Também é comum aceitarem cartões de crédito para parcelamento da entrada ou do valor total.';
                console.log('[UA] Parágrafo FAQ3 substituído.');
            }
        }

        // 4. "Sou pessoa física. Consigo comprar no portal do lojista?"
        var faq4 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--header__F04Mr .LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq4 && faq4.innerText.trim() === 'Sou pessoa física. Consigo comprar no portal do lojista?') {
            if (faq4.innerText !== 'Como posso verificar a procedência e documentação do carro?') {
                faq4.innerText = 'Como posso verificar a procedência e documentação do carro?';
                console.log('[UA] FAQ4 substituído.');
            }
        }
        var p4Div = document.querySelector('div:has(> p.LdsTypography-module_lds-typography--body-md__KI9TK)');
        if (p4Div) {
            var p4 = p4Div.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
            if (p4 && p4.innerText.trim() === 'Não, se você é uma pessoa física e deseja comprar um veículo seminovo, acesse o site') {
                // Substituir o texto e remover o link
                p4.innerText = 'Todos os veículos comercializados passam por uma verificação de procedência. No momento da compra, o cliente tem acesso às informações sobre multas, IPVA e restrições. A transferência de propriedade segue o rito padrão do DETRAN, e a loja fornece o suporte necessário para que o documento seja transferido corretamente para o novo proprietário após a quitação.';
                var link = p4Div.querySelector('a');
                if (link) link.remove();
                console.log('[UA] Parágrafo FAQ4 substituído e link removido.');
            }
        }

        // 5. "Consigo fazer compras 100% online?"
        var faq5 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq5 && faq5.innerText.trim() === 'Consigo fazer compras 100% online?') {
            if (faq5.innerText !== 'Posso dar o meu carro atual como entrada na troca?') {
                faq5.innerText = 'Posso dar o meu carro atual como entrada na troca?';
                console.log('[UA] FAQ5 substituído.');
            }
        }
        var p5 = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (p5 && p5.innerText.trim() === 'Sim, você poderá comprar o seu veículo totalmente online e sem sair de casa. Realize já o seu cadastro para ter acesso a essa funcionalidade.') {
            if (p5.innerText !== 'Sim, a loja avalia veículos como parte do pagamento. Vale lembrar que, como a Universal trabalha com margens de repasse, a avaliação do seu usado também seguirá uma métrica de mercado para revenda rápida, permitindo que você saia com um modelo mais novo ou de categoria superior utilizando seu crédito atual.') {
                p5.innerText = 'Sim, a loja avalia veículos como parte do pagamento. Vale lembrar que, como a Universal trabalha com margens de repasse, a avaliação do seu usado também seguirá uma métrica de mercado para revenda rápida, permitindo que você saia com um modelo mais novo ou de categoria superior utilizando seu crédito atual.';
                console.log('[UA] Parágrafo FAQ5 substituído.');
            }
        }

        // 6. "Quais são as formas de pagamento?"
        var faq6 = document.querySelector('.LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span');
        if (faq6 && faq6.innerText.trim() === 'Quais são as formas de pagamento?') {
            if (faq6.innerText !== 'Onde a loja física está localizada e qual o horário de atendimento?') {
                faq6.innerText = 'Onde a loja física está localizada e qual o horário de atendimento?';
                console.log('[UA] FAQ6 substituído.');
            }
        }
        var p6 = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (p6 && p6.innerText.trim() === 'O pagamento pode ser realizado via PIX à vista ou boleto bancário.') {
            if (p6.innerText !== 'A sede física fica em Santo André - SP, na Avenida Pereira Barreto, 42 - Vila Gilda. O atendimento ocorre de segunda a sexta, das 9h às 16h, e aos sábados também das 9h às 16h. É recomendável agendar uma visita ou consultar o estoque atual via WhatsApp antes de se deslocar, devido à alta rotatividade dos veículos.') {
                p6.innerText = 'A sede física fica em Santo André - SP, na Avenida Pereira Barreto, 42 - Vila Gilda. O atendimento ocorre de segunda a sexta, das 9h às 16h, e aos sábados também das 9h às 16h. É recomendável agendar uma visita ou consultar o estoque atual via WhatsApp antes de se deslocar, devido à alta rotatividade dos veículos.';
                console.log('[UA] Parágrafo FAQ6 substituído.');
            }
        }

        // 7. Último parágrafo "Os melhores preços para a sua empresa..."
        var pFinal = document.querySelector('p.LdsTypography-module_lds-typography--body-md__KI9TK');
        if (pFinal && pFinal.innerText.trim() === 'Os melhores preços para a sua empresa, com flexibilidade de negociação.') {
            if (pFinal.innerText !== 'Os melhores preços do mercado, com flexibilidade de negociação.') {
                pFinal.innerText = 'Os melhores preços do mercado, com flexibilidade de negociação.';
                console.log('[UA] Parágrafo final substituído.');
            }
        }
    }
                ...
                if (mutation.addedNodes && mutation.addedNodes.length) {
                    mutation.addedNodes.forEach(function(node) {
                        if (node.nodeType === 1 && (node.matches && node.matches('.LdsDialog-module_lds-dialog-container__Br8cE'))) {
                            ...
                            var innerModal = node.querySelector('.LdsDialog-module_lds-dialog-container__Br8cE');
                            if (innerModal) modifyLoginDialog();
                        }
                    });
                }
            });
    ...
    // ============================================================
    // MUTATION OBSERVER COM DEBOUNCE (geral)
    // ============================================================
    var debounceTimer;
    ...
    function isPageFullyTransformed() {
        ...
        var tituloPrincipal = document.querySelector('h1.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-xl__xfa9y');
        if (tituloPrincipal && tituloPrincipal.innerText.trim() !== 'Seu próximo carro está aqui. Conheça as vantagens do nosso estoque.') {
            console.log('[UA] Aguardando substituicao do título principal...');
            return false;
        }

        var heroImg = document.querySelector('img.Hero_heroImage__14M5B[alt="Hero Image"]');
        if (heroImg && heroImg.src && heroImg.src.includes('/home/computer.webp')) {
            console.log('[UA] Aguardando substituicao da imagem hero...');
            return false;
        }
        ...
        if (verMaisBtn) {
            console.log('[UA] Aguardando modificacao do botao Ver mais carros...');
            return false;
        }
        ...
        if (howItWorksSection) {
            ...
            if (!computedBg.includes('linear-gradient') || computedBg.includes('rgb(30, 133, 45)')) {
                console.log('[UA] Aguardando alteracao do background da seção HowItWorks...');
                return false;
            }
        }
        ...
        if (primeiroH2 && primeiroH2.innerText.trim() === 'Faça seu cadastro') {
            console.log('[UA] Aguardando substituicao dos textos dos passos...');
            return false;
        }
        ...
        if (cadastroHowItWorks && cadastroHowItWorks.innerText.trim() === 'Cadatre-se agora') {
            console.log('[UA] Aguardando substituicao do botão Cadatre-se agora...');
            return false;
        }

        var equipeDiv = document.querySelector('.EfficiencyForBusiness_teamContainer__8bJzm');
        if (equipeDiv) {
            var equipeBg = window.getComputedStyle(equipeDiv).background;
            if (!equipeBg.includes('linear-gradient')) {
                console.log('[UA] Aguardando background da equipe especializada...');
                return false;
            }
        }
        ...
        var faqDiv = document.querySelector('.FAQ_banner__7TtUF');
        if (faqDiv) {
            var faqBg = window.getComputedStyle(faqDiv).background;
            if (!faqBg.includes('linear-gradient')) {
                console.log('[UA] Aguardando background da FAQ...');
                return false;
            }
            var faqTitle = faqDiv.querySelector('h2');
            if (faqTitle && faqTitle.innerText.trim() !== 'Gostaria de conhecer as viaturas disponíveis em exposição?') {
                console.log('[UA] Aguardando título da FAQ...');
                return false;
            }
        }
    ...
    console.log('[UA] Script v12 ativo. Mascara de pagina ativa. Loader aguarda transformacoes.');
})();