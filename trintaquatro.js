(async function universalAutoScript() {
    ...
    // ============================================================
    // [FIX 2] MASCARA SINCRONA MOBILE — protege elementos mobile
    // PERMANENTEMENTE até cada um ser transformado individualmente.
    // Esta style NÃO é removida pelo removeLoader — persiste sempre.
    // Cada função mobile define data-ua-done="1" (ou data-ua-filter-done)
    // no elemento após transformá-lo, revelando-o de forma controlada.
    // ============================================================
    const _mobileMask = document.createElement('style');
    _mobileMask.id = 'ua-mobile-mask';
    _mobileMask.textContent = [
        /* Logo mobile SVG — oculto até ser substituído */
        '.DisplayContent_hideDesktop__hlDJO:not([data-ua-done]){opacity:0!important;transition:none!important;pointer-events:none!important;}',
        /* Imagem hero mobile — oculta até src ser trocado */
        'img.Hero_heroImageMobile__TXyFn:not([data-ua-done]){opacity:0!important;transition:none!important;}',
        /* Secção de interesse mobile — oculta até WhatsApp ser injectado */
        '.MobilePage_contentInterest__dY5dv:not([data-ua-done]){opacity:0!important;transition:none!important;pointer-events:none!important;}',
        /* Botão hamburger — removido permanentemente via display:none */
        'button[aria-label="abrir menu"]{display:none!important;}',
        /* Container de botões do filtro mobile — oculto até "Entrar" ser removido */
        '.VehiclesFilters_buttonsContainer__lY4oa:not([data-ua-filter-done]){opacity:0!important;transition:none!important;}'
    ].join('');
    (document.head || document.documentElement).appendChild(_mobileMask);
    ...
    console.log('[UA] Universal Auto Script (v23 - Mobile Zero Flash) iniciado...');
    ...
    (function() {
        ...
        function removeLoader() {
            ...
            // Remove apenas a máscara global — a máscara mobile (ua-mobile-mask)
            // NÃO é removida aqui; cada função mobile remove a sua própria protecção
            // via atributo data-ua-done após concluir a transformação.
            ...
            if (loadingDiv && loadingDiv.parentNode) {
                ...
                setTimeout(function() { if (loadingDiv && loadingDiv.parentNode) loadingDiv.parentNode.removeChild(loadingDiv); }, 600);
            }
        ...
        setTimeout(function() { if (!loaderRemoved) { clearInterval(checkInterval); removeLoader(); } }, TIMEOUT_MS + 2000);
        console.log('[UA] Loader ativo. A aguardar transformacoes.');
    })();
    ...
    function modifyCadastroButton() {
        ...
        var botoes = heroContainer.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk');
    ...
    function replaceMainTitle() {
        var titulos = document.querySelectorAll('h1.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-xl__xfa9y');
        titulos.forEach(function(titulo) {
            if (titulo.innerText.trim() === 'Diversos modelos, com todas as vantagens Localiza!') {
    ...
    function modifyHowItWorksBackground() {
        ...
        if (section) {
            ...
            if (section.style.background !== targetGradient) {
    ...
    function modifySeparatorsBackground() {
        var targetGradient = 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)';
        document.querySelectorAll('.HowItWorks_leftBorder__P6mDf,.HowItWorks_rightBorder__JevRe').forEach(function(el) {
    ...
    function replaceHowItWorksTexts() {
        ...
        steps.forEach(function(step) {
            document.querySelectorAll('h2.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-md__QVgP4').forEach(function(h2) {
                if (h2.innerText.trim() === step.h2Text && h2.innerText !== step.newH2) h2.innerText = step.newH2;
            });
            document.querySelectorAll('p.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--body-md__KI9TK').forEach(function(p) {
                if (p.innerText.trim() === step.pText && p.innerText !== step.newP) p.innerText = step.newP;
            });
        });
    }
    ...
    function modifyEquipeEspecializadaBackground() {
        ...
        if (div) {
            var targetGradient = 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)';
            if (!window.getComputedStyle(div).background.includes(targetGradient)) {
                div.style.background = targetGradient;
                div.style.setProperty('background', targetGradient, 'important');
            }
        }
    }
    ...
    function modifyFAQSection() {
        ...
        if (!faqDiv) return;
        var targetGradient = 'linear-gradient(135deg, rgb(37, 99, 235) 0%, rgb(30, 64, 175) 50%, rgb(30, 58, 138) 100%)';
        if (!window.getComputedStyle(faqDiv).background.includes(targetGradient)) {
            faqDiv.style.background = targetGradient;
            faqDiv.style.setProperty('background', targetGradient, 'important');
        }
        var h2 = faqDiv.querySelector('h2.LdsTypography-module_lds-typography--heading-md__QVgP4');
        if (h2 && h2.innerText.trim() === 'Não possui CNPJ e quer um Seminovo Localiza?') {
            h2.innerText = 'Gostaria de conhecer as viaturas disponíveis em exposição?';
        }
        ...
        if (linkP && linkP.innerText.trim() === 'https://seminovos.localiza.com/') {
            linkP.innerText = 'https://universalautorepasse.com.br/';
        }
        var link = faqDiv.querySelector('a');
        if (link && link.getAttribute('href') === 'https://seminovos.localiza.com/') {
            link.setAttribute('href', 'https://universalautorepasse.com.br/');
        }
    }
    ...
    function modifyFaqCadastroButton() {
        ...
        faqButtons.querySelectorAll('button.LdsButton-module_lds-button--contained-primary__6r3Mk').forEach(function(btn) {
            var span = btn.querySelector('span');
            if (!span) return;
            if (span.innerText.trim() === 'Cadastre-se agora' && !btn.dataset.uaFaqDone) {
    ...
    function modifyLoginDialog() {
        ...
        if (p && p.innerText.trim() === 'Faça o Login ou cadastre-se para ter acesso a todas nossas ofertas') {
            p.innerText = 'Consulte o nosso catálogo e utilize os filtros ou a barra de pesquisa para localizar as melhores oportunidades desta marca.';
        }
    ...
    function modifyContatoButton() {
        ...
        botoes.forEach(function(btn) {
            ...
            var span = btn.querySelector('span');
            if (!span) return;
            if (span.innerText.trim() === 'Veja o nosso catálogo') {
                span.innerText = 'Nosso Contacto';
                var novoBtn = btn.cloneNode(true);
                btn.parentNode.replaceChild(novoBtn, btn);
    ...
    function modifyHowItWorksTitle() {
        ...
        if (titleElement) {
            const newHTML = 'Como funciona? <br> Veja como é <span>rápido</span> <br> e <span>simples</span> comprar na universal.';
    ...
    const respostasMapping = [
        { original: "Após realizar o cadastro, você passará a ter acesso completo ao catálogo",
          novo: "Comprar um carro de repasse significa adquirir um veículo no estado em que ele se encontra, geralmente proveniente de trocas em grandes concessionárias. Esses carros são vendidos por valores significativamente abaixo da tabela FIPE porque a loja não realiza revisões estéticas ou mecânicas completas antes da venda, repassando a margem de lucro e a responsabilidade de manutenção para o comprador." },
        { original: "Você poderá cadastrar apenas empresas que possuam um CNAE principal",
          novo: "Nesta modalidade específica de negócio, os veículos são vendidos sem garantia de mecânica ou estética. O comprador assume o carro \"no estado\", ciente de que o preço reduzido compensa eventuais manutenções que precisem ser feitas. Por isso, recomendamos sempre levar um mecânico de confiança para avaliar o veículo no pátio antes de fechar o negócio." },
        { original: "Para acessar as ofertas disponíveis é necessário que você realize o cadastro",
          novo: "Sim, a Universal Auto Repasse costuma trabalhar com parcerias bancárias para facilitar o pagamento. No entanto, por serem carros com preços promocionais, as condições de financiamento e a aprovação de crédito dependem do ano do veículo e do perfil do CPF do cliente. Também é comum aceitarem cartões de crédito para parcelamento da entrada ou do valor total." },
        { original: "Não, se você é uma pessoa física e deseja comprar um veículo seminovo",
    ...
    function replaceFaqAnswers() {
        let replacedCount = 0;
        const paragraphs = document.querySelectorAll('.LdsAccordionItem-module_lds-accordion-item--content__ns-Tz p.LdsTypography-module_lds-typography--body-md__KI9TK');
        paragraphs.forEach(p => {
            ...
            for (let item of respostasMapping) {
                if (current === item.original || current.startsWith(item.original)) {
                    if (p.innerText !== item.novo) {
                        p.innerText = item.novo;
                        replacedCount++;
                    }
                    break;
                }
    ...
    function modifyAccordionFaq() {
        ...
        document.querySelectorAll('p.LdsTypography-module_lds-typography--body-md__KI9TK').forEach(p => {
            if (p.innerText.trim() === 'Os melhores preços para a sua empresa, com flexibilidade de negociação.') {
            ...
            for (const mut of mutations) {
                if (mut.addedNodes.length) {
                    for (const node of mut.addedNodes) {
                        if (node.nodeType === 1) {
                            if (node.matches && (node.matches('.LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span') || node.matches('.LdsAccordionItem-module_lds-accordion-item--content__ns-Tz p'))) {
                                needsReplace = true;
                            } else if (node.querySelector && (node.querySelector('.LdsAccordionItem-module_lds-accordion-item--title__n6Vx5 span') || node.querySelector('.LdsAccordionItem-module_lds-accordion-item--content__ns-Tz p'))) {
                                needsReplace = true;
                            }
                        }
                    }
                ...
                if (mut.type === 'characterData' && mut.target.parentNode) {
                    ...
                    if ((parentTitle && parentTitle.closest('.LdsAccordionItem-module_lds-accordion-item--title__n6Vx5')) ||
                        (parentPara && parentPara.closest('.LdsAccordionItem-module_lds-accordion-item--content__ns-Tz'))) {
                        needsReplace = true;
                    }
                }
            }
            ...
            mutations.forEach(function(mutation) {
                if (mutation.addedNodes && mutation.addedNodes.length) {
                    mutation.addedNodes.forEach(function(node) {
                        if (node.nodeType === 1 && node.matches && node.matches('.LdsDialog-module_lds-dialog-container__Br8cE')) {
                            modifyLoginDialog();
                        } else if (node.nodeType === 1 && node.querySelector) {
                            var inner = node.querySelector('.LdsDialog-module_lds-dialog-container__Br8cE');
    ...
    function applySidebar(carroData) {
        ...
        container.innerHTML = '<div class="VehicleTitle_container__mDy6V">'
            + '<div class="VehicleTitle_header__bZBhS"><h4 class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--heading-sm__5NKKy VehicleTitle_version__d4gVg">' + carroData.brand + '</h4></div>'
            ...
            + '<p class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--body-md__KI9TK">' + carroData.manufactureYear + '/' + carroData.modelYear + '&nbsp;&nbsp;&nbsp;&nbsp;' + km + ' km</p></div>'
            ...
            + '<p class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--body-sm__7uR1-">' + nomeLoja + '<br><span class="VehicleLocation_opacity__7fAgZ">' + cidade + '</span></p></div>'
            + '<div style="background-color:var(--divider-color);height:1px;width:100%"></div>'
            + '<div class="VehiclePrice_container__myGEh"><div class="Price_priceContainer__UFleO"><div><h2 class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--heading-lg__-4owW">' + precoFormatado + '</h2></div></div></div>'
            + '<div class="whatsapp-button-wrapper" style="margin-bottom:24px;"><button type="button" id="whatsapp-button-script" class="whatsapp-button-custom"><img src="' + WHATSAPP_ICON_URL + '" alt="WhatsApp" class="whatsapp-icon" style="width:24px;height:24px;margin-right:8px;vertical-align:middle;"><span>WhatsApp</span></button></div>'
            ...
            + '<img src="https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/logoalternativo.png" alt="Logo Universal" style="margin-top:20px;width:150px;height:auto;"></div>';
    }
    ...
    function applyLogo() {
        document.querySelectorAll('.DisplayContent_displayContent__I14hU.DisplayContent_hideMobile__K13cl.DisplayContent_hideTablet__ISpUr.DisplayContent_hideLargeTablet__jiADA').forEach(function(div) {
            ...
            if (svgEl && svgEl.getAttribute('viewBox') === '0 0 149 39' && div.innerHTML.includes('#FF8026') && !div.querySelector('img')) {
                div.innerHTML = '<div style="display:flex;align-items:center;gap:8px;"><img src="https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/logoalternativo.png" alt="Universal Repasses" style="height:39px;width:auto;object-fit:contain;"><div style="display:flex;flex-direction:column;line-height:1.15;"><span style="font-size:10px;color:#1565C0;font-weight:600;letter-spacing:2px;text-transform:uppercase;">Repasses</span><span style="font-size:17px;color:#0D3B8C;font-weight:800;letter-spacing:0.5px;">Universal</span></div></div>';
            }
        });
    }
    ...
    // ============================================================
    // [MOBILE] FUNÇÃO 1: Substitui logo SVG mobile no navbar
    // Após transformar → data-ua-done="1" revela o elemento via CSS
    // ============================================================
    function applyMobileNavLogo() {
        var mobileLogoDivs = document.querySelectorAll(
            '.DisplayContent_displayContent__I14hU.DisplayContent_hideDesktop__hlDJO'
        );
        mobileLogoDivs.forEach(function(div) {
            // Já transformado
            if (div.getAttribute('data-ua-done') === '1') return;

            var svgEl = div.querySelector('svg[viewBox="0 0 39 39"]');
            if (!svgEl) {
                // Sem SVG original mas também sem img nossa → marca como feito se já tiver img
                if (div.querySelector('img[data-ua-mobile-logo]')) div.setAttribute('data-ua-done', '1');
                return;
            }

            // Redireciona o link pai para universalauto.xyz clonando-o para remover listeners
            var parentLink = div.closest('a');
            if (parentLink) {
                parentLink.setAttribute('href', 'https://universalauto.xyz');
                parentLink.setAttribute('title', 'Universal Auto');
                parentLink.removeAttribute('data-testid');
                var clonedLink = parentLink.cloneNode(true);
                parentLink.parentNode.replaceChild(clonedLink, parentLink);
                clonedLink.addEventListener('click', function(e) {
                    e.preventDefault();
                    window.location.href = 'https://universalauto.xyz';
                });
                var newDiv = clonedLink.querySelector('.DisplayContent_hideDesktop__hlDJO');
                if (newDiv) {
                    newDiv.innerHTML = '<img data-ua-mobile-logo="1" src="https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/logoalternativo.png" alt="Universal Auto" style="height:39px;width:auto;object-fit:contain;display:block;">';
                    // Revela: o CSS ua-mobile-mask só oculta sem data-ua-done
                    newDiv.setAttribute('data-ua-done', '1');
                }
            } else {
                div.innerHTML = '<img data-ua-mobile-logo="1" src="https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/logoalternativo.png" alt="Universal Auto" style="height:39px;width:auto;object-fit:contain;display:block;">';
                div.setAttribute('data-ua-done', '1');
            }
            console.log('[UA] Logo mobile substituído e revelado.');
        });
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 2: Substitui secção de interesse mobile
    // Após transformar → data-ua-done="1" revela o elemento via CSS
    // ============================================================
    function applyMobileInterestSection() {
        var interestDivs = document.querySelectorAll('.MobilePage_contentInterest__dY5dv');
        ...
        interestDivs.forEach(function(div) {
            ...
            if (carroId && dadosCarregados && precoPorId.has(carroId)) {
                var preco = precoPorId.get(carroId);
                var precoFormatado = preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
                precoHtml = '<div style="text-align:center;margin-bottom:10px;font-size:26px;font-weight:800;color:#1e40af;letter-spacing:-0.5px;font-family:inherit;">'
                    + precoFormatado + '</div>';
            }

            div.innerHTML = precoHtml
                + '<button type="button" class="whatsapp-button-custom">'
            ...
            // Revela o elemento — o CSS :not([data-ua-done]) deixa de ocultar
            div.setAttribute('data-ua-done', '1');
            console.log('[UA] Secção interesse mobile transformada e revelada.');
        });
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 3: Substitui imagem hero mobile
    // Após transformar → data-ua-done="1" revela o elemento via CSS
    // ============================================================
    function applyMobileHeroImage() {
        var mobileHero = document.querySelector('img.Hero_heroImageMobile__TXyFn');
        if (!mobileHero) return;
        if (mobileHero.getAttribute('data-ua-done') === '1') return;
        ...
        if (src.includes('computer-mobile.webp') || src === '/home/computer-mobile.webp') {
            ...
            // Revela a imagem após substituição
            mobileHero.setAttribute('data-ua-done', '1');
            console.log('[UA] Imagem hero mobile substituída e revelada.');
        } else if (!src.includes('computer-mobile.webp')) {
            // Src já foi trocado noutro ciclo, garante que está revelado
            mobileHero.setAttribute('data-ua-done', '1');
        }
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 4: Remove botão hamburger menu
    // O CSS ua-mobile-mask já o oculta com display:none permanente.
    // Esta função remove-o do DOM para limpeza.
    // ============================================================
    function removeHamburgerMenu() {
        var hamburger = document.querySelector('button[aria-label="abrir menu"]');
        if (hamburger) {
            hamburger.remove();
            console.log('[UA] Botão hamburger removido do DOM.');
        }
    }

    // ============================================================
    // [MOBILE] FUNÇÃO 5: Remove botões do container de filtros mobile
    // Após remover → data-ua-filter-done="1" revela o container via CSS
    // ============================================================
    function removeEntrarFromFilterModal() {
        var containers = document.querySelectorAll('.VehiclesFilters_buttonsContainer__lY4oa');
        containers.forEach(function(container) {
            // Já processado
            if (container.getAttribute('data-ua-filter-done') === '1') return;

            container.querySelectorAll('button').forEach(function(btn) {
                var txt = btn.innerText.trim();
                if (txt === 'Entrar' || txt === 'Cadastrar') {
                    btn.remove();
                    console.log('[UA] Botão "' + txt + '" removido do modal filtros mobile.');
                }
            });

            // Revela o container — o CSS :not([data-ua-filter-done]) deixa de ocultar
            container.setAttribute('data-ua-filter-done', '1');
        });
    }

    // ============================================================
    // [MOBILE] OBSERVER: Detecta adição do modal de filtros ao DOM
    // e processa-o ANTES de ser pintado — zero flash garantido
    // ============================================================
            ...
            mutations.forEach(function(mut) {
                ...
                mut.addedNodes.forEach(function(node) {
                    ...
                    if (node.classList && node.classList.contains('VehiclesFilters_buttonsContainer__lY4oa')) {
                        ...
                        targets = Array.from(node.querySelectorAll('.VehiclesFilters_buttonsContainer__lY4oa'));
                    }
                    // O CSS ua-mobile-mask já os oculta via :not([data-ua-filter-done]).
                    // Processamos e marcamos data-ua-filter-done para os revelar.
                    if (targets.length) removeEntrarFromFilterModal();
                });
            });
        });
        obs.observe(document.body, { childList: true, subtree: true });
        console.log('[UA] Observer do modal de filtros mobile activo.');
    }
    ...
    // ============================================================
    // MUTATION OBSERVER GERAL (debounce 150ms)
    // ============================================================
    var debounceTimer;
    ...
    // ============================================================
    // EVENT LISTENER GLOBAL PARA WHATSAPP
    // ============================================================
    ...
    function isPageFullyTransformed() {
        ...
        if (!document.getElementById('universal-auto-styles')) return false;
        ...
        if (cadastrarAindaExiste) return false;
        ...
        if (logoOriginalAindaExiste) return false;

        var tituloPorSubstituir = Array.from(
            document.querySelectorAll('h1.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-xl__xfa9y')
        ).some(function(t) { return t.innerText.trim() === 'Diversos modelos, com todas as vantagens Localiza!'; });
        if (tituloPorSubstituir) return false;

        var heroImg = document.querySelector('img.Hero_heroImage__14M5B[alt="Hero Image"]');
        if (heroImg && heroImg.src && heroImg.src.includes('/home/computer.webp')) return false;

        // [MOBILE] Imagem hero mobile ainda por transformar
        var mobileHero = document.querySelector('img.Hero_heroImageMobile__TXyFn');
        if (mobileHero) {
            var mSrc = mobileHero.getAttribute('src') || mobileHero.src || '';
            if (mSrc.includes('computer-mobile.webp')) return false;
        }

        // [MOBILE] Logo mobile SVG ainda por substituir
        var mobileLogoSvgExiste = Array.from(
            document.querySelectorAll('.DisplayContent_hideDesktop__hlDJO svg')
        ).some(function(s) { return s.getAttribute('viewBox') === '0 0 39 39'; });
        if (mobileLogoSvgExiste) return false;

        // [MOBILE] Hamburger ainda presente (o CSS oculta mas devemos remover)
        if (document.querySelector('button[aria-label="abrir menu"]')) return false;

        // [MOBILE] Botão "Entrar" ainda no container de filtros
        var entrarNoFiltro = Array.from(
            document.querySelectorAll('.VehiclesFilters_buttonsContainer__lY4oa button')
        ).some(function(b) { return b.innerText.trim() === 'Entrar'; });
        if (entrarNoFiltro) return false;

        var verMaisBtn = Array.from(document.querySelectorAll('button.LdsButton-module_lds-button--outlined-primary__ZxRfx')).some(function(btn) {
            var span = btn.querySelector('span');
            return span && span.innerText.trim() === 'Ver mais carros';
        });
        if (verMaisBtn) return false;

        var howItWorksSection = document.querySelector('section.HowItWorks_container__IYMRX');
        if (howItWorksSection) {
            var computedBg = window.getComputedStyle(howItWorksSection).background;
            if (!computedBg.includes('linear-gradient') || computedBg.includes('rgb(30, 133, 45)')) return false;
        }

        var primeiroH2 = document.querySelector('h2.LdsTypography-module_lds-typography__-DOlx.LdsTypography-module_lds-typography--heading-md__QVgP4');
        if (primeiroH2 && primeiroH2.innerText.trim() === 'Faça seu cadastro') return false;

        var cadastroHowItWorks = document.querySelector('section.HowItWorks_container__IYMRX button.LdsButton-module_lds-button--contained-primary__6r3Mk span');
        if (cadastroHowItWorks && cadastroHowItWorks.innerText.trim() === 'Cadatre-se agora') return false;

        var equipeDiv = document.querySelector('.EfficiencyForBusiness_teamContainer__8bJzm');
        if (equipeDiv && !window.getComputedStyle(equipeDiv).background.includes('linear-gradient')) return false;

        var faqDiv = document.querySelector('.FAQ_banner__7TtUF');
        if (faqDiv) {
            if (!window.getComputedStyle(faqDiv).background.includes('linear-gradient')) return false;
            var faqTitle = faqDiv.querySelector('h2');
            if (faqTitle && faqTitle.innerText.trim() !== 'Gostaria de conhecer as viaturas disponíveis em exposição?') return false;
        }

        if (isDetalhePage) {
            var sidebarExiste = !!document.querySelector('.DesktopPage_rightSide__uZfet .DesktopPage_container__eiImn');
            if (sidebarExiste && !document.getElementById('whatsapp-button-script')) return false;

            // [MOBILE] Secção de interesse mobile sem WhatsApp
            var mobileInterest = document.querySelector('.MobilePage_contentInterest__dY5dv');
            if (mobileInterest && mobileInterest.getAttribute('data-ua-done') !== '1') return false;
        }
        ...
        if (isListagemPage && dadosCarregados) {
            ...
            cards.forEach(function(card) {
                ...
                cardsComLink++;
                var precoElem = card.querySelector('[class*="blurred"]');
                if ((!precoElem || precoElem.style.filter === 'none') && card.querySelector('.botao-personalizado')) cardsOk++;
            });
            if (cardsComLink > 0 && (cardsOk / cardsComLink) < 0.8) return false;
        }

        console.log('[UA] Pagina completamente transformada!');
        return true;
    }
    ...
    var aggressiveRetryInterval = setInterval(function() {
        ...
        if (isPageFullyTransformed()) {
            ...
            if (typeof window.markTransformationsComplete === 'function') window.markTransformationsComplete();
            return;
        }
        verificationCount++;
        if (verificationCount > 45) {
            clearInterval(aggressiveRetryInterval);
            console.warn('[UA] Limite de verificacoes atingido.');
            if (typeof window.markTransformationsComplete === 'function') window.markTransformationsComplete();
        }
    }, 200);

    console.log('[UA] Script v23 (Mobile Zero Flash) ativo.');
})();