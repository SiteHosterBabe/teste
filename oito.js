(async function universalAutoScript() {
    ...
    // ========== NOVA LÓGICA: SÓ SINALIZA QUANDO AS TRANSFORMAÇÕES ESTIVEREM PRONTAS ==========
    // Verifica periodicamente se o botão "Cadastrar" foi removido (indica que as mudanças foram aplicadas)
    let verificationCount = 0;
    const verificationInterval = setInterval(() => {
        const cadastrarExiste = Array.from(document.querySelectorAll('button')).some(b => b.innerText.trim() === 'Cadastrar');
        const precoModificado = !cadastrarExiste; // indicador simples
        if (!cadastrarExiste) {
        ...
        verificationCount++;
        if (verificationCount > 30) { // ~9 segundos (30 * 300ms)
            clearInterval(verificationInterval);
