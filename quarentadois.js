(async function universalAutoScript() {
    ...
    console.log('[UA] Script v32 - Botões fora do link, posicionados corretamente');

    // ============================================================
    // LOADER PREMIUM (resumido, igual ao anterior)
    // ============================================================
    (function() {
        ...
        window.markTransformationsComplete = function() { console.log('[UA] Transformacoes completas.'); };
    })();
    ...
    function createModal(title, contentElement, width = '900px') {
        ...
        modalContainer.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(0,0,0,0.5);z-index:999999;display:flex;align-items:center;justify-content:center;';
    ...
    function showPdfModal(pdfUrl) {
        ...
        loadingDiv.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;color:#0A66C2;font-family:sans-serif;';
    ...
    // ============================================================
    // FUNÇÕES AUXILIARES DE TRANSFORMAÇÃO (resumidas, mas completas)
    // ============================================================
    function applyHero() {
    ...
    // ============================================================
    // FUNÇÃO PRINCIPAL DE LISTAGEM (COM BOTÕES FORA DO LINK E POSICIONADOS)
    // ============================================================
    function applyListagem() {
        ...
        cards.forEach(function(card) {
            ...
            // 3. BOTÕES LAUDO E VÍDEO - REMOVER DO LINK E POSICIONAR FORA
            ...
            if (hasPdf || hasVideo) {
                // Procura o container de botões existente dentro do link
                var oldContainer = link.querySelector('.VehicleCard_badgeContainer__v1N5b');
                var newContainer;
                if (oldContainer) {
                    // Remove do link e guarda
                ...
                // Preenche o container (se não processado)
                if (!newContainer.hasAttribute('data-ua-processed')) {
                ...
                // INSERIR O CONTAINER FORA DO LINK, MAS DENTRO DO CARD, NA POSIÇÃO CORRETA
                // Encontramos o footer (último elemento do link) e inserimos antes dele, mas fora do link?
                // Na verdade, queremos que o container fique visualmente entre o infoContainer e o footer.
                // Como o container é irmão do link, precisamos posicioná-lo absolutamente.
                // Vamos colocar o container como irmão do link e usar CSS para posicionar.
                // Para isso, precisamos que o card tenha position: relative.
                if (!card.style.position) card.style.position = 'relative';
                newContainer.style.position = 'absolute';
                // Calcular a posição top baseada no infoContainer
                ...
                if (infoContainer) {
                    ...
                    var topRelative = infoRect.bottom - cardRect.top + 8; // 8px de margem
                    newContainer.style.top = topRelative + 'px';
                    newContainer.style.left = '16px';
                    newContainer.style.right = '16px';
                    newContainer.style.zIndex = '2';
                }
                card.appendChild(newContainer); // Agora o container é filho direto do card (fora do link)
            }
    ...
    function applySidebar(carroData) {
        var rightSidebar = document.querySelector('.DesktopPage_rightSide__uZfet'); if (!rightSidebar || !carroData) return;
        var container = rightSidebar.querySelector('.DesktopPage_container__eiImn'); if (!container) return; if (container.querySelector('#whatsapp-button-script')) return;
        var novoPreco = carroData.priceFor + ACRESCIMO; var precoFormatado = novoPreco.toLocaleString('pt-BR', { style:'currency', currency:'BRL' });
        var nomeLoja = (carroData.store && carroData.store.name) ? carroData.store.name : 'Loja não informada';
        var cidade = (carroData.store && carroData.store.city && carroData.store.state) ? carroData.store.city + ' - ' + carroData.store.state : 'Fortaleza - CE';
        var km = carroData.odometer.toLocaleString('pt-BR');
        container.innerHTML = '<div class="VehicleTitle_container__mDy6V"><div class="VehicleTitle_header__bZBhS"><h4 class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--heading-sm__5NKKy VehicleTitle_version__d4gVg">' + carroData.brand + '</h4></div><h2 class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--heading-lg__-4owW VehicleTitle_model__Ec3VK"><strong>' + carroData.modelFamilyDescription + '</strong> ' + carroData.model + '</h2><p class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--body-md__KI9TK">' + carroData.manufactureYear + '/' + carroData.modelYear + '&nbsp;&nbsp;&nbsp;&nbsp;' + km + ' km</p></div><div class="VehicleReports_container__qAkuy"></div><div style="background-color:var(--divider-color);height:1px;width:100%"></div><div class="VehicleLocation_container__m8i4B"><svg width="20" height="20" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" fill="none" role="presentation" class="location-icon"><path d="m5.0001 2.09998c-.3409 0-.65253.1926-.80499.4975l-2 4c-.06248.12497-.09501.26278-.09501.4025v1c0 1.03434.41089 2.02632 1.14228 2.75772.25691.2569.54597.4743.85772.6482v8.6941h-1.1c-.49706 0-.9.4029-.9.9 0 .497.40294.9.9.9h18c.4971 0 .9-.403.9-.9 0-.4971-.4029-.9-.9-.9h-1.1v-8.6941c.3117-.1739.6008-.3913.8577-.6482.7314-.7314 1.1423-1.72338 1.1423-2.75772v-1c0-.13972-.0325-.27753-.095-.4025l-2-4c-.1525-.3049-.4641-.4975-.805-.4975zm.9 18.00002v-8.2013c.03328.0008.06662.0013.1.0013 1.03434 0 2.02632-.4109 2.75771-1.1423.08516-.0852.16597-.1738.24229-.2657.07632.0919.15713.1805.24228.2657.73139.7314 1.72342 1.1423 2.75772 1.1423s2.0263-.4109 2.7577-1.1423c.0852-.0852.166-.1738.2423-.2657.0763.0919.1571.1805.2423.2657.7314.7314 1.7234 1.1423 2.7577 1.1423.0334 0 .0667-.0005.1-.0013v8.2013h-2.2v-3.1c0-.7692-.3055-1.5068-.8494-2.0506-.5438-.5439-1.2815-.8494-2.0506-.8494h-2c-.7691 0-1.50676.3055-2.05061.8494-.54386.5438-.84939 1.2814-.84939 2.0506v3.1zm4.6151-10.6151c-.3939-.39383-.6151-.92797-.6151-1.48492v-.1h4.2v.1c0 .55695-.2213 1.09109-.6151 1.48492s-.9279.6151-1.4849.6151-1.0911-.22127-1.4849-.6151zm-2.4151-1.48492c0 .55695-.22125 1.09109-.61508 1.48492-.39382.39383-.92797.6151-1.48492.6151-.55696 0-1.0911-.22127-1.48493-.6151-.39382-.39383-.61507-.92797-.61507-1.48492v-.1h4.2zm-3.64377-1.9 1.1-2.2h12.88757l1.1 2.2zm11.44377 1.8h4.2v.1c0 .55695-.2213 1.09109-.6151 1.48492s-.9279.6151-1.4849.6151-1.0911-.22127-1.4849-.6151c-.3939-.39383-.6151-.92797-.6151-1.48492zm-4.9 8.00002h2c.2917 0 .5715.1159.7778.3222.2063.2062.3222.486.3222.7778v3.1h-4.2v-3.1c0-.2918.1159-.5716.3222-.7778.2063-.2063.4861-.3222.7778-.3222z" fill="currentColor"></path></svg><p class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--body-sm__7uR1-">' + nomeLoja + '<br><span class="VehicleLocation_opacity__7fAgZ">' + cidade + '</span></p></div><div style="background-color:var(--divider-color);height:1px;width:100%"></div><div class="VehiclePrice_container__myGEh"><div class="Price_priceContainer__UFleO"><div><h2 class="LdsTypography-module_lds-typography__-DOlx LdsTypography-module_lds-typography--heading-lg__-4owW">' + precoFormatado + '</h2></div></div></div><div class="whatsapp-button-wrapper" style="margin-bottom:24px;"><button type="button" id="whatsapp-button-script" class="whatsapp-button-custom"><img src="' + WHATSAPP_ICON_URL + '" alt="WhatsApp" class="whatsapp-icon" style="width:24px;height:24px;margin-right:8px;vertical-align:middle;"><span>WhatsApp</span></button></div><div id="banner-container" style="display:flex;flex-direction:column;justify-content:center;align-items:center;padding:0;background:white;border-radius:0;border:none!important;box-shadow:none!important;"><img src="https://raw.githubusercontent.com/SiteHosterBabe/universalauto/refs/heads/main/banneruniversal.png" alt="Banner Universal" style="width:100%;height:auto;max-height:600px;object-fit:contain;"><img src="https://universalautorepasse.com.br/wp-content/uploads/2024/10/Universal-Veiculos-e-Repasses-300x275.png" alt="Logo Universal" style="margin-top:20px;width:150px;height:auto;"></div>';
    }
    ...
    function applyAllTransformations() {
        try {
            ...
            var idMatch = window.location.href.match(/-(\d+)(?:\/|$)/); var carroId = idMatch ? parseInt(idMatch[1]) : null;
            if (carroId && todosVeiculos.length) { var carroData = todosVeiculos.find(function(c) { return c.id === carroId; }); if (carroData) applySidebar(carroData); }
    ...
    let debounceTimer; const observer = new MutationObserver(() => { clearTimeout(debounceTimer); debounceTimer = setTimeout(applyAllTransformations, 150); }); observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });
    ...
    console.log('[UA] Script v32 ativo. Botões fora do link e posicionados corretamente. Modais com loading.');
})();