(async function universalAutoScript() {
    ...
    console.log('🚀 Universal Auto Script (v5) iniciado...');

    // --- 0. CRIA A TELA DE LOADING IMEDIATAMENTE ---
    function createLoadingOverlay() {
        ...
        return { overlay, progressBar, statusText };
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.warn('⚠️ JSON não carregado.');
        }
    } catch (err) {
        console.error('❌ Erro ao carregar JSON:', err);
    }

    // --- 2. FUNÇÕES DE TRANSFORMAÇÃO (IDEMPOTENTES) ---
    function applyHero() {
    ...
    // --- 3. FUNÇÃO PRINCIPAL QUE APLICA TUDO ---
    ...
    // --- 4. MUTATION OBSERVER E INTERVALO AGRESSIVO PARA CARREGAMENTO TARDIO ---
    ...
    const retryInterval = setInterval(() => {
        applyAllTransformations();
        intervalCount++;
        if (intervalCount > 10) clearInterval(retryInterval); // ~5 segundos (10 * 500ms)
    }, 500);

    // --- 5. FINALIZA O LOADING APÓS 7 SEGUNDOS ---
    ...
    console.log('✅ Universal Auto Script v5 - Loading corrigido e transformações agressivas.');
})();